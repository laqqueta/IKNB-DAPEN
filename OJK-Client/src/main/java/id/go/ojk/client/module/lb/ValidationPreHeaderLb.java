...
public class ValidationPreHeaderLb extends BaseValidationPreHeader {
	...
	public ValidationPreHeaderLb(File file, int sumRow, int reportFormGroupCode, SubmissionFormat submissionFormat,
	...
	protected boolean validatePart6() {
		...
		boolean res = false;
		String code = String.valueOf(EReportGroupLb.getRevisionCodeByCode(reportFormGroupCode));
	...
	protected boolean validatePart7() {
		String sectorCode = ctx.getService(SetupService.class).getSectorCode();
		int maxLen = ESector.isPnj(sectorCode) ? 50 : 25;
