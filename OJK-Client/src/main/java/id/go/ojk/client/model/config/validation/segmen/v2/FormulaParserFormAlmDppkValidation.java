...
@XStreamAlias("FormulaParserFormAlmDppkValidation")
public class FormulaParserFormAlmDppkValidation extends BaseRowValidation {
	...
	public FormulaParserFormAlmDppkValidation() {
		super();
	}

	public FormulaParserFormAlmDppkValidation(String parameter) {
		super(parameter);
	}

	@Override
	public FormulaParserFormAlmDppkValidation initialized() {
		super.initialized();
	...
	public void validate(SubmissionData submissionData, SubmissionFormat submissionFormat, ValidationResult validationResult) {
		...
		final Logger logger = LoggerFactory.getLogger(FormulaParserFormAlmDppkValidation.class);
		...
		for (FormulaParser.Group g : parsed.getGroups()) {
			...
			for (FormulaParser.RowToken t : g.getTokens()) {
				...
				for (int i = 0; i < t.getOperands().size(); i++) {
					...
					SubmissionFormat.getStreamOfFormSubMap(comparatorForm + t.getRowCode(), Character.MAX_VALUE)
							.forEach(v -> {
								...
								rowTotal.set(calculateOperand(rowTotal.get(), fieldValue, "+"));
							});
					...
					if (fieldErrorMessages.length > 0) errBuilder.append(fieldErrorMessages[i]).append("'");
