...
public class FormulaParserPeriodePelaporanFormValidation extends BaseRowValidation {
	protected String comparatorForm;
	protected String operationForm;
	protected MessageType messageType;
	protected int scale;

	public FormulaParserPeriodePelaporanFormValidation() {
		super();
	}

	public FormulaParserPeriodePelaporanFormValidation(String parameter) {
		super(parameter);
	}

	@Override
	public FormulaParserPeriodePelaporanFormValidation initialized() {
		super.initialized();
		...
		for (FormulaParser.Group g : parsed.getGroups()) {
			...
			for (FormulaParser.RowToken t : g.getTokens()) {
				...
				for (int i = 0; i < t.getOperands().size(); i++) {
					...
					if (isInvalidNumeric(formValue.get(o.getField()))) fieldValue = BigDecimal.ZERO;
					else fieldValue = new BigDecimal(formValue.get(o.getField()));
