...
@Slf4j
public class ValidationFileNamePls2 extends ValidationFileName2 {

	public ValidationFileNamePls2(List<ValidFile> files, ReportInfo reportInfo, String memberCode,
			String memberTypeCode) {
		super(files, reportInfo, memberCode, memberTypeCode);
	}

	public ValidationFileNamePls2(List<ValidFile> files, ReportInfo reportInfo, String memberCode,
			String memberTypeCode, ProgressPreparationAndSending childProgress, ReadSubmissionDirModel readDirModel) {
	...
	protected boolean validatePart4(ValidFile file, String firstPeriodReport, MonthDate firstMonthDate,
		...
		if (part.length() != 8) {
			...
			if (!firstMonthDate.getMapMonthDate().containsKey(month)) {
				...
				} else if (periodeType.equals("Q")) {
					if (!validateQuarterly(part)) {
						listError.add("Nama file '" + fileName + "', Tanggal Periode Data Pelaporan '" + part
								+ "' tidak valid. Posisi tanggal harus tanggal 31 Maret atau 30 Juni atau 30 September atau 31 Desember");
					} else {
						res = true;
					}
				} else if (periodeType.equals("M")) {
					res = true;
				}
			}
		}
	...
	private boolean validateQuarterly(String part) {
		return part.endsWith("0331") || part.endsWith("0630") || part.endsWith("0930") || part.endsWith("1231");
	}
