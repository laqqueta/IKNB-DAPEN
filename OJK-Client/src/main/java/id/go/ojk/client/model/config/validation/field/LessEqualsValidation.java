...
public class LessEqualsValidation extends MapParamFieldValidation {
	...
	public void validate(SubmissionFormat submissionFormat, SubmissionField field, ValidationResult validationResult) {
		Logger logger = LoggerFactory.getLogger(LessEqualsValidation.class);
		...
		if (arrFieldComparator != null) {
			for (int i = 0; i < arrFieldComparator.length; i++) {
				...
				if (ToBigDecimal.convert(realValue).compareTo(ToBigDecimal.convert(comparatorValue)) > 0) {
					logger.error("{}|{}?{}", validationResult.getColumn(1), realValue, comparatorValue);
					validationResult.errors.add(new ValidationError(field, ValidationErrorCode.E11_09_LESS_EQUAL,
