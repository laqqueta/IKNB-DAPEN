...
@XStreamAlias("maxLengthValidation")
public class MaxLengthValidation extends BaseRowValidation {
	...
	public MaxLengthValidation(String parameter) {
		super(parameter);
	}
	...
	@Override
	public MaxLengthValidation initialized() {
		super.initialized();
	...
	public void validate(SubmissionData submissionData, SubmissionFormat submissionFormat,
		...
		if (selectPosCode.contains(posCode)) {
			for (String field : arrSelectField) {
				...
				if (!fieldValueString.equals("") && !isMustEmpty(submissionFormat, validationResult, submissionField)
						&& (fieldValueString.length() > maxLength)) {
