...
public class ValidationFileNameSa extends BaseValidationFileName {

	public ValidationFileNameSa(List<File> files, ReportInfo reportInfo, ReadSubmissionDirModel readDirModel) {
	...
	public IValidationFile<ValidFile> initValidation2() {
		return new ValidationFileNameSa2(getListFile(), reportInfo, memberCode, memberTypeCode, childProgress,
				readDirModel);
	}

	@Override
	public IValidationFile<ValidFile> initValidation3() {
		return new ValidationFileNameSa3(getListFile(), reportInfo, memberCode, memberTypeCode, listSubmissionFormat,
				mainProgress, childProgress, readDirModel);
	}

	@Override
	protected ReportGroup getReportGroup(int reportFormGroupCode) {
		return EReportGroupSa.getObjectByCode(reportFormGroupCode);
	}
