...
@XStreamAlias("SumIfOffsetFormValidation")
public class SumIfOffsetFormValidation extends BaseSumIf<SumIfBaseParams> {

    private String multiRangeFieldErr;

    public SumIfOffsetFormValidation(String parameter) {
        super(parameter);
    }

    @Override
    public SumIfOffsetFormValidation initialized() {
        super.initialized();
    ...
    public void validate(SubmissionData submissionData, SubmissionFormat submissionFormat, ValidationResult validationResult) {
        ...
        if (selectRowCodes.contains(currentRowCode)) {
            ...
            SumIfBaseParams param = new SumIfBaseParams(conds, rangeField);
