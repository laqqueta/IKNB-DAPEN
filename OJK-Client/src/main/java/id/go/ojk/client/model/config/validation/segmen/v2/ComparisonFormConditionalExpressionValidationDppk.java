...
@XStreamAlias("ComparisonFormConditionalExpressionValidationDppk")
@Slf4j
/*
 * Use case:
 * If A in form 1 is <expr1>,then SUM(A:C) in form 2 must [eq atau neq> <expr2>
 */
public class ComparisonFormConditionalExpressionValidationDppk extends ComparisonFormValidationV2 {
    private String selectExpr;
    private String comparatorExpr;

    public ComparisonFormConditionalExpressionValidationDppk() {
        super();
    }

    public ComparisonFormConditionalExpressionValidationDppk(String parameter) {
        super(parameter);
    }
    ...
    public String genMessage() {
        if (StringUtils.isNotBlank(msgError)) {
            ...
            if (components.length == 2) {
                msgError = "Jika '" + components[0] + "' " + comparatorExpr + ", maka '" + components[1] + comparatorForm + "' harus " + selectExpr;
            }
            return msgError;
        } else {
