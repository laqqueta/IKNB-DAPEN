...
import id.go.ojk.client.vc.HomeMetroBpjsControllerLBBPTK;
import id.go.ojk.client.vc.HomeMetroBpjsControllerLPBPTK;
import id.go.ojk.client.vc.HomeMetroBpjsControllerLTBPTK;
...
public class FormBpjsTk extends BaseCustomForm {
  ...
  public FormBpjsTk(ApplicationContext appContext, MainApplication mainApplication) {
    ...
    formLt = new FormBpjsTkBase<HomeMetroBpjsControllerLTBPTK>(appContext, mainApplication,
        "vc/HomeMetroBpjsLTBPTK.fxml");
    init();
  }
  ...
  public void ensureVisibleScrollPane(ProgressSegment progressSegment, int menuCode) {
    ...
    formLt.ensureVisibleScrollPane(progressSegment);
  }

  public void showForm(DisplayScene scene) {
    if (scene.toString().startsWith(LBBPTK)) {
    ...
    } else if (scene.toString().startsWith(LTBPTK)) {
      formLt.showForm(scene);
    }
