...
@XStreamAlias("GreaterEqualsValidation")
public class GreaterEqualsValidation extends BaseRowValidation {
	private String comparatorRowCode;

	public GreaterEqualsValidation() {
		super();
	}

	public GreaterEqualsValidation(String parameter) {
		super(parameter);
	}
	...
	@Override
	public GreaterEqualsValidation initialized() {
		super.initialized();
	...
	public void validate(SubmissionData submissionData, SubmissionFormat submissionFormat,
		...
		if (selectPosCode.contains(posCode)) {
			for (String field : arrSelectField) {
				...
				if (!isGreaterEquals(fieldValueString, comparator)) {
					SubmissionField submissionField = fields.get(Integer.parseInt(field));
					validationResult.errors.add(new ValidationError(submissionField, ValidationErrorCode.E11_11_GREATER_EQUAL, comparator));
				}
			}
		}
	}
	
	private boolean isGreaterEquals(String value, BigDecimal comparator) {
