...
public class ValidationFileNameLbk1 extends ValidationFileName1 {

	public ValidationFileNameLbk1(List<File> files, ReportInfo reportInfo) {
		super(files, reportInfo);
	}

	public ValidationFileNameLbk1(List<File> files, ReportInfo reportInfo, ReadSubmissionDirModel readDirModel) {
	...
	protected boolean validateExtention(ValidFile validFile, List<String> listError) {
		...
		for (int i = 0; i < listExtension.size(); i++) {
			res = extension.equalsIgnoreCase(listExtension.get(i));
			if (res) {
				break;
			}
		}
