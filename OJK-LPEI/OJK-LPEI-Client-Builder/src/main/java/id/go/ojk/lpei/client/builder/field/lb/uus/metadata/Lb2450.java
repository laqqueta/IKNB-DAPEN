...
import id.go.ojk.lpei.client.builder.field.lb.uus.reference.ER7019PosLbLpeiu2450;
...
public class Lb2450 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulananUus eEnum = EFormLaporanBulananUus.LB_2450;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);

		res.setSavePosForm(ER7019PosLbLpeiu2450.genFieldSave());

		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLbUus.R7019_LB2450.getObject()).confRegex(patternNumeric)));
		...
		fs.add(sf(10, null, "Kualitas Aset",
				sv(M, 1, 10, refTable).confReference(EHeaderMetadataShared.R017.getObject())
						.confRegex(SimpleValidation.PATTERN_REFERENCE_1.c())));
		fs.add(sf(11, null, "Nominal, Dalam Mata Uang Asal", sv(M, 1, 23, numericDot)));
		fs.add(sf(12, null, "Nominal, Dalam Ekuivalen Rupiah", sv(M, 1, 20, numeric)));
		fs.add(sf(13, null, "Persentage Terhadap Modal", sv(M, 1, 10, numericDot)));
		fs.add(sf(14, null, "PPKA", sv(M, 1, 20, numeric)));
		return res;
	}
}
