...
import id.go.ojk.lpei.client.builder.field.lb.uus.reference.ER7015PosLbLpeiu2200;
...
public class Lb2200 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		res.setSavePosForm(ER7015PosLbLpeiu2200.genFieldSave());

		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLb.R5015_LB2200.getObject()).confRegex(patternNumeric)));
		...
		fs.add(sf(12, null, "Tujuan Kepemilikan",
				sv(M, 1, 10, refTable).confReference(EHeaderMetadataShared.R023.getObject())
						.confRegex(SimpleValidation.PATTERN_REFERENCE_1.c())));
		fs.add(sf(13, null, "Akad yang Digunakan",
				sv(M, 1, 10, refTable).confReference(EHeaderMetadataShared.R037.getObject())
						.confRegex(SimpleValidation.PATTERN_REFERENCE_1.c())));
		...
		fs.add(sf(16, null, "Kualitas Aset",
				sv(M, 1, 10, refTable).confReference(EHeaderMetadataShared.R017.getObject())
						.confRegex(SimpleValidation.PATTERN_REFERENCE_1.c())));
		fs.add(sf(17, null, "Suku Bunga/Bagi Hasil/Imbal Hasil - Jenis",
				sv(M, 1, 10, refTable).confReference(EHeaderMetadataShared.R015.getObject())
						.confRegex(SimpleValidation.PATTERN_REFERENCE_1.c())));
		fs.add(sf(18, null, "Suku Bunga/Bagi Hasil/Imbal Hasil - Nilai", sv(M, 1, 20, numeric)));
		fs.add(sf(19, null, "Suku Bunga/Bagi Hasil/Imbal Hasil - Tingkat", sv(M, 1, 10, DataType.numericDot)));
		fs.add(sf(20, null, "Nominal Saldo Akhir - Dalam Mata Uang Asal", sv(M, 1, 23, numericDot)));
		fs.add(sf(21, null, "Nominal Saldo Akhir - Dalam Ekuivalen Rupiah", sv(M, 1, 20, numeric)));
		fs.add(sf(22, null, "Harga Perolehan", sv(M, 1, 20, numeric)));
		fs.add(sf(23, null, "Premium/Diskonto", sv(M, 1, 20, numericNegatif)));
		fs.add(sf(24, null, "PPKA", sv(M, 1, 10, numeric)));
		return res;
	}
}
