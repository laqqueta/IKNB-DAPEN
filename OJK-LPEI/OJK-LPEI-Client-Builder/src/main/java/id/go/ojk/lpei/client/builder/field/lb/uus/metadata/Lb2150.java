...
public class Lb2150 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLbUus.R7014_LB2150.getObject()).confRegex(patternNumeric)));
		fs.add(sf(2, null, "Nomor Peminjam", sv(M, 1, 100, all)));
		fs.add(sf(3, null, "Nama Peminjam", sv(M, 1, 100, all)));
		fs.add(sf(4, null, "Nama Kelompok Peminjam", sv(M, 1, 10, all)
		...
		fs.add(sf(13, null, "Akad yang Digunakan", sv(M, 1, 10, refTable)
				...
				.confRegex(SimpleValidation.PATTERN_REFERENCE_1.c())));
		fs.add(sf(14, null, "Jangka Waktu, Tanggal Mulai", sv(M, 1, 8, date)));
		fs.add(sf(15, null, "Jangka Waktu, Tanggal Jatuh Tempo", sv(M, 1, 8, date)));
		fs.add(sf(16, null, "Nilai Awal Pembiayaan", sv(M, 1, 20, numeric)));
		...
		fs.add(sf(21, null, "Ujrah, Tingkat", sv(M, 1, 10, numericDot)));
		...
		fs.add(sf(24, null, "Kualitas Aset", sv(M, 1, 10, refTable)
				...
				.confRegex(SimpleValidation.PATTERN_REFERENCE_1.c())));
		fs.add(sf(25, null, "Pembayaran Angsuran Terakhir, Tanggal", sv(M, 1, 8, date)));
		fs.add(sf(26, null, "Pembayaran Angsuran Terakhir, Angsuran Ke-", sv(M, 1, 10, numeric)));
		fs.add(sf(27, null, "Pembayaran Angsuran Terakhir, Nilai Angsuran", sv(M, 1, 20, numeric)));
		fs.add(sf(28, null, "PPKA", sv(M, 1, 20, numeric)));
		fs.add(sf(29, null, "Proporsi Asuransi Kredit atau Penjaminan Kredit", sv(M, 1, 10, numericDot)));
		fs.add(sf(30, null, "Nama Perusahaan Asuransi/ Penjaminan", sv(M, 1, 100, all)));
		fs.add(sf(31, null, "Jangka Waktu Asuransi/ Penjaminan", sv(M, 1, 20, numeric)));
