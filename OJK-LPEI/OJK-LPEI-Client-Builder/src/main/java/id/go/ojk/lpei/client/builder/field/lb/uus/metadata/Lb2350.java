...
import id.go.ojk.lpei.client.builder.field.lb.uus.reference.ER7017PosLbLpeiu2350;
...
public class Lb2350 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		res.setSavePosForm(ER7017PosLbLpeiu2350.genFieldSave());

		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLbUus.R7017_LB2350.getObject()).confRegex(patternNumeric)));
		...
		fs.add(sf(4, null, "Tanggal Penutupan", sv(M, 1, 8, date)));
		fs.add(sf(5, null, "Total Nilai Reasuransi", sv(M, 1, 20, numeric)));
		fs.add(sf(6, null, "Pengajuan Klaim, Tanggal Klaim", sv(M, 1, 8, date)));
		fs.add(sf(7, null, "Pengajuan Klaim, Total Klaim", sv(M, 1, 20, numeric)));
		fs.add(sf(8, null, "Pembayaran Klaim, Tanggal Pembayaran", sv(M, 1, 8, date)));
