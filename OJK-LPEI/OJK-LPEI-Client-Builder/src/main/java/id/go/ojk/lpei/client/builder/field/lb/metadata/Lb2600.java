...
import id.go.ojk.lpei.client.builder.field.lb.reference.ER5022PosLbLpeig2600;
...
public class Lb2600 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		res.setRequiredPos(ER5022PosLbLpeig2600.getRequiredPos());
		res.setSavePosForm(ER5022PosLbLpeig2600.genFieldSave());

		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLb.R5022_LB2600.getObject())
		...
		fs.add(sf(6, null, "Kreditur - Nama", sv(M, 1, 100, all)));
		fs.add(sf(7, null, "Kreditur - Negara", sv(M, 1, 10, refTable)
				.confReference(EHeaderMetadataShared.R024.getObject())
				.confRegex(SimpleValidation.PATTERN_REFERENCE_3.c())));
		fs.add(sf(8, null, "Kreditur - Golongan", sv(M, 1, 100, all)));
		fs.add(sf(9, null, "Jangka Waktu - Tanggal Mulai", sv(M, 8, 8, date)));
		fs.add(sf(10, null, "Jangka Waktu - Tanggal Selesai", sv(M, 8, 8, date)));
		fs.add(sf(11, null, "Suku Bunga/Bagi Hasil/Imbal Hasil - Jenis", sv(M, 1, 10, refTable)
				.confReference(EHeaderMetadataShared.R015.getObject())
				.confRegex(SimpleValidation.PATTERN_REFERENCE_1.c())));
		fs.add(sf(12, null, "Suku Bunga/Bagi Hasil/Imbal Hasil - Nilai", sv(M, 1, 20, numeric)));
		fs.add(sf(13, null, "Suku Bunga/Bagi Hasil/Imbal Hasil - Tingkat", sv(M, 1, 10, DataType.numericDot)));
		fs.add(sf(14, null, "Nilai Nominal Surat Berharga - Dalam Mata Uang Asal", sv(M, 1, 20, DataType.numericDot)));
		fs.add(sf(15, null, "Nilai Nominal Surat Berharga - Dalam Ekuivalen Rupiah", sv(M, 1, 20, numeric)));
		fs.add(sf(16, null, "Premium/Diskonto", sv(M, 1, 20, DataType.numericNegatif)));
		fs.add(sf(17, null, "Saldo Surat Berharga yang Diterbitkan - Dalam Mata Uang Asal", sv(M, 1, 20, DataType.numericDot)));
		fs.add(sf(18, null, "Saldo Surat Berharga yang Diterbitkan - Dalam Ekuivalen Rupiah", sv(M, 1, 20, numeric)));
		fs.add(sf(19, null, "Status Keterkaitan", sv(M, 1, 10, refTable)
		...
		fs.add(sf(23, null, "Lembaga Pemeringkat", sv(M, 1, 10, refTable)
				.confReference(EHeaderMetadataShared.R025.getObject())
				.confRegex(SimpleValidation.PATTERN_REFERENCE_1.c())));
		fs.add(sf(24, null, "Peringkat Surat Berharga", sv(M, 1, 10, refTable)
				.confReference(EHeaderMetadataShared.R026.getObject())
				.confRegex(SimpleValidation.PATTERN_REFERENCE_1.c())));
		fs.add(sf(25, null, "Tanggal Pemeringkatan", sv(M, 8, 8, date)));
		fs.add(sf(26, null, "Ketersediaan Sinking Fund", sv(M, 1, 10, refTable)
				.confReference(EHeaderMetadataShared.R059.getObject())
				.confRegex(SimpleValidation.PATTERN_REFERENCE_1.c())));
		return res;
	}
}
