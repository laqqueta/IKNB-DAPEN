...
import id.go.ojk.lpei.client.builder.field.lb.reference.ER5020PosLbLpeig2490;
...
public class Lb2490 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		res.setRequiredPos(ER5020PosLbLpeig2490.getRequiredPos());
		res.setSavePosForm(ER5020PosLbLpeig2490.genFieldSave());

		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLb.R5020_LB2490.getObject())
		...
		fs.add(sf(4, null, "Nominal - Dalam Mata Uang Asal", sv(O, 1, 20, DataType.numericDot)));
		fs.add(sf(5, null, "Nominal - Dalam Ekuivalen Rupiah", sv(M, 1, 20, numeric)));
		fs.add(sf(6, null, "Cadangan Kerugian Penurunan Nilai - Metode Secara Individual", sv(M, 1, 20, numeric)));
		fs.add(sf(7, null, "Cadangan Kerugian Penurunan Nilai - Metode Secara Kolektif", sv(M, 1, 20, numeric)));
		fs.add(sf(8, null, "Cadangan Kerugian Penurunan Nilai - Aset Baik", sv(M, 1, 20, numeric)));
		fs.add(sf(9, null, "Cadangan Kerugian Penurunan Nilai - Aset Kurang Baik", sv(M, 1, 20, numeric)));
		fs.add(sf(10, null, "Cadangan Kerugian Penurunan Nilai - Aset Tidak Baik", sv(M, 1, 20, numeric)));
		return res;
	}
}
