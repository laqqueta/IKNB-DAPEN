...
import id.go.ojk.lpei.client.builder.field.lb.uus.reference.ER7018PosLbLpeiu2400;
...
public class Lb2400 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		res.setSavePosForm(ER7018PosLbLpeiu2400.genFieldSave());

		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLbUus.R7018_LB2400.getObject()).confRegex(patternNumeric)));
		fs.add(sf(2, null, "Nomor Referensi Transaksi (Deal Number)", sv(M, 1, 100, all)));
		fs.add(sf(3, null, "Jenis", sv(M, 1, 10, refTable).confReference(EHeaderMetadataShared.R029.getObject())
		...
		fs.add(sf(9, null, "Kualitas Aset", sv(M, 1, 10, refTable).confReference(EHeaderMetadataShared.R017.getObject())
				.confRegex(SimpleValidation.PATTERN_REFERENCE_1.c())));
		fs.add(sf(10, null, "Jumlah Tagihan", sv(M, 1, 20, numeric)));
		fs.add(sf(11, null, "PPKA", sv(M, 1, 20, numeric)));
		return res;
	}
}
