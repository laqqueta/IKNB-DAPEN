...
import id.go.ojk.lpei.client.builder.field.lb.uus.reference.ER7026PosLbLpeiu2790;
...
public class Lb2790 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulananUus eEnum = EFormLaporanBulananUus.LB_2790;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);

		res.setSavePosForm(ER7026PosLbLpeiu2790.genFieldSave());

		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLbUus.R7026_LB2790.getObject()).confRegex(patternNumeric)));
		...
		fs.add(sf(5, null, "Jangka Waktu, Tanggal Mulai", sv(O, 1, 8, date)));
		fs.add(sf(6, null, "Jangka Waktu, Tanggal Selesai", sv(O, 1, 8, date)));
