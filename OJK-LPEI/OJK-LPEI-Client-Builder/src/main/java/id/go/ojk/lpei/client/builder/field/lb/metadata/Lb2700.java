...
import id.go.ojk.lpei.client.builder.field.lb.reference.ER5023PosLbLpeig2700;
...
public class Lb2700 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		res.setRequiredPos(ER5023PosLbLpeig2700.getRequiredPos());
		res.setSavePosForm(ER5023PosLbLpeig2700.genFieldSave());

		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLb.R5023_LB2700.getObject())
				.confRegex(patternNumeric)));
		fs.add(sf(2, null, "Nomor Referensi Transaksi (Deal Number)", sv(M, 1, 100, all)));
		fs.add(sf(3, null, "Jenis Transaksi Derivatif", sv(M, 1, 10, refTable)
