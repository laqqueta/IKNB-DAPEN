...
import id.go.ojk.pnjs.client.builder.field.lb.reference.ER3010PosLbPnjs2100B2;
...
public class Lb2100B2 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulanan eEnum = EFormLaporanBulanan.LB_2100B2;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null, ER3010PosLbPnjs2100B2.genFieldSave(), null, ER3010PosLbPnjs2100B2.getRequiredPos());

        res.addSegmentValidations(ER3010PosLbPnjs2100B2.genValidation1());
        res.addSegmentValidations(ER3010PosLbPnjs2100B2.genValidationTotal());

		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLb.R3010_LB2100B2.getObject())));
		fs.add(sf(2, null, "Jenis Surat Berharga", sv(C, 1, 10, refTable).confRegex(PATTERN_REFERENCE_1)
                ...
                .confConditionalRequired(ER3010PosLbPnjs2100B2.genConditionTotal())));
		...
		fs.add(sf(10, null, "Jangka Waktu-Tanggal Penerbitan", sv(C, 8, 8, date)
                .confConditionalRequired(ER3010PosLbPnjs2100B2.genConditionTotal()))
				.addFieldValidations(ER3010PosLbPnjs2100B2.genFieldValidation10()));
		fs.add(sf(11, null, "Jangka Waktu-Tanggal Jatuh Tempo", sv(C, 8, 8, date)
                .confConditionalRequired(ER3010PosLbPnjs2100B2.genConditionTotal()))
				.addFieldValidations(ER3010PosLbPnjs2100B2.genFieldValidation11()));
		fs.add(sf(12, null, "Suku Bunga atau Kupon-Tingkat Suku Bunga atau Kupon", sv(C, 3, 6, numericDot)
                .confConditionalRequired(ER3010PosLbPnjs2100B2.genConditionTotal()))
				.addFieldValidations(ER3010PosLbPnjs2100B2.genFieldValidationGreater("0.00")));
		...
		fs.add(sf(18, null, "CKPN-Individual",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3010PosLbPnjs2100B2.genCondition("19"))));
		fs.add(sf(19, null, "CKPN-Kolektif",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3010PosLbPnjs2100B2.genCondition("18"))));
		fs.add(sf(20, null, "Jumlah Neto", sv(M, 1, 16, numericNegatif))
				.addFieldValidations(ER3010PosLbPnjs2100B2.genFieldValidation20()));
		return res;
	}
}
