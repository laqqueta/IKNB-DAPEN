...
import id.go.ojk.pnjs.client.Constant;
import id.go.ojk.pnjs.client.builder.field.EUpdateHeader;
import id.go.ojk.pnjs.client.builder.field.setup.ESetupKeyValueStringFilename;
import lombok.extern.slf4j.Slf4j;

@Slf4j
public class PnjsDatBuilder extends BaseDatBuilder {
	private static final EReport REPORT_LB = EReport.LB_PNJS;
	private static final List<String> DUMMY_USER = Arrays.asList("dev.external05@yahoo.com", "dev@local");

	public static void main(String[] args) {
		try {
			log.info("starting builder");
			Constant.initAppVersion();
			PnjsDatBuilder builder = new PnjsDatBuilder(args);
	...
	public PnjsDatBuilder(String[] args) {
	...
	protected String genDummyClientEntity() {
		return genDummyClientEntity(Constant.SECTOR.getCode(), Arrays.asList("2600000099"), DUMMY_USER);
	}
