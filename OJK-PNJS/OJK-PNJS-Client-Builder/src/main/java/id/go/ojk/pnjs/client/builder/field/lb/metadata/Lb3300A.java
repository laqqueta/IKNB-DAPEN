...
import id.go.ojk.pnjs.client.builder.field.lb.reference.ER3028PosLbPnjs3300A;
...
public class Lb3300A extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulanan eEnum = EFormLaporanBulanan.LB_3300A;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLb.R3028_LB3300A.getObject())));
		fs.add(sf(2, null, "Lokasi Penerima Jaminan (Makfuul Lahu)-(Dati II)", sv(M, 1, 10, refTable)
		...
		fs.add(sf(18, null, "Total-Jumlah Terjamin (Makfuul Lahu)-(Satuan)", sv(M, 1, 8, numeric))
				.addFieldValidations(ER3028PosLbPnjs3300A.genFieldValidation18()));
		fs.add(sf(19, null, "Total-Nilai Kafalah (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3028PosLbPnjs3300A.genFieldValidation19()));
		fs.add(sf(20, null, "Total-Nilai Klaim (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3028PosLbPnjs3300A.genFieldValidation20()));
		return res;
	}
}
