...
import id.go.ojk.pnjs.client.builder.field.lb.reference.ER3024PosLbPnjs3000;
...
public class Lb3000 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulanan eEnum = EFormLaporanBulanan.LB_3000;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLb.R3024_LB3000.getObject())));
		...
		fs.add(sf(5, null, "Tingkat Suku Bunga", sv(M, 3, 6, numericDot))
				.addFieldValidations(ER3024PosLbPnjs3000.genFieldValidation5()));
		fs.add(sf(6, null, "Jangka Waktu-Mulai", sv(M, 8, 8, date)));
		fs.add(sf(7, null, "Jangka Waktu-Jatuh Tempo", sv(O, 8, 8, date))
				.addFieldValidations(ER3024PosLbPnjs3000.genFieldValidation7()));
		fs.add(sf(8, null, "Jangka Waktu-Tenor", sv(M, 1, 8, numeric))
				.addFieldValidations(ER3024PosLbPnjs3000.genFieldValidationGreater("1")));
		return res;
	}
}
