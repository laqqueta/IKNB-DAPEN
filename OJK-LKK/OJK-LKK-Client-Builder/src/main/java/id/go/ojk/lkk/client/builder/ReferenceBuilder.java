...
import id.go.ojk.lpei.client.builder.field.setup.EMessage;
...
public class ReferenceBuilder extends BaseReferenceBuilder {
	...
	private Map<String, List<KeyValueString>> getReferences() {
		...
		res.putAll(LpeiConfiguration.getReferences());
		res.putAll(PnmkConfiguration.getReferences());
		return res;
	}
	...
	private List<ReferenceMetadata> getReferenceMetadata() {
		...
		List<Integer> listReference = new ArrayList<>();
		for (ReferenceMetadata metadata : LpeiConfiguration.getReferenceMetadata()) {
			if (!listReference.contains(metadata.getNumber())) {
				res.add(metadata);
				listReference.add(metadata.getNumber());
			}
		}
		for (ReferenceMetadata metadata : PnmkConfiguration.getReferenceMetadata()) {
			if (!listReference.contains(metadata.getNumber())) {
				res.add(metadata);
				listReference.add(metadata.getNumber());
			}
