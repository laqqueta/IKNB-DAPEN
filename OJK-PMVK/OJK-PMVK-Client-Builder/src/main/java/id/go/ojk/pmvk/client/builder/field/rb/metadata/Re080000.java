...
import id.go.ojk.pmvk.client.builder.field.rb.reference.ER2259PosRePmvk080000;
	...
	public SubmissionFormat get() {
		...
		fs.add(sf(3, null, "Jenis Kantor",
				sv(C, 0, 10, refTable).confReference(EHeaderMetadataShared.R006.getObject())
						.confConditionalRequired(ER2259PosRePmvk080000.genConditionalField3()))
				.addFieldValidations(ER2259PosRePmvk080000.genValidationField3()));
		fs.add(sf(4, null, "Waktu Pelaksanaan",
				sv(C, 0, 6, yearMonth).confConditionalRequired(ER2259PosRePmvk080000.genConditionalField4())));
		fs.add(sf(5, null, "Rencana Investasi/Biaya",
				sv(C, 0, 16, numericNegatif).confConditionalRequired(ER2259PosRePmvk080000.genConditionalField5())));
		fs.add(sf(6, null, "Realisasi Investasi/Biaya",
				sv(C, 0, 16, numericNegatif).confConditionalRequired(ER2259PosRePmvk080000.genConditionalField6())));
		fs.add(sf(7, null, "Deviasi",
				sv(C, 0, 16, numericNegatif).confConditionalRequired(ER2259PosRePmvk080000.genConditionalField7()))
				.addFieldValidations(ER2259PosRePmvk080000.genValidationField7()));
		fs.add(sf(8, null, "Lokasi",
				sv(C, 0, 10, refTable).confConditionalRequired(ER2259PosRePmvk080000.genConditionalField8())
						.confReference(EHeaderMetadataShared.R002.getObject())));
		fs.add(sf(9, null, "Keterangan",
				sv(C, 0, 1000, all).confConditionalRequired(ER2259PosRePmvk080000.genConditionalField9())));
		return res;
	}
}
