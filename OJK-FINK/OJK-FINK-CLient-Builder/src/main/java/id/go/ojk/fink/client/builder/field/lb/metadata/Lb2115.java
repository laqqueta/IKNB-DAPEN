...
import id.go.ojk.fink.client.builder.field.lb.reference.ER3021PosLbFink2115;

public class Lb2115 extends BaseMetadata {

	public Lb2115(String reportCode) {
		super(reportCode, ".txt");
	}

	@Override
	public SubmissionFormat get() {
		EFormLaporanBulanan eEnum = EFormLaporanBulanan.LB_2115;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 1, null, ER3021PosLbFink2115.genFieldSave(), ER3021PosLbFink2115.genFieldSaveForm(),
				ER3021PosLbFink2115.getRequiredPos());
		res.addSegmentValidations(ER3021PosLbFink2115.genRowValidation2());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 12, 12, refTable)
				.confReference(EHeaderMetadataLb.R3021_LB2115.getObject()).confRegex(patternNumeric)));
		fs.add(sf(2, null, "Nama Kreditur", sv(C, 1, 100, all).confRegex(patternLJKName)
				.confConditionalRequired(ER3021PosLbFink2115.genConditional())));
		...
		fs.add(sf(5, null, "Jumlah Utang Jangka Panjang Lainnya", sv(M, 1, 16, numericNegatif))
				.addFieldValidations(ER3021PosLbFink2115.genFieldValidation5()));
		fs.add(sf(6, null, "Tanggal Pencairan",
				sv(C, 1, 8, date).confConditionalRequired(ER3021PosLbFink2115.genConditional())));
