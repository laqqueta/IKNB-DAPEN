...
import id.go.ojk.fink.client.builder.field.lb.reference.ER3031PosLbFink2911;
...
public class Lb2911 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 1, null, ER3031PosLbFink2911.genFieldSave(), null, ER3031PosLbFink2911.getRequiredPos());
		res.addSegmentValidations(ER3031PosLbFink2911.genRowValidation2());
		...
		fs.add(sf(2, null, "Nama Pihak Ketiga", sv(C, 1, 100, all).confRegex(patternLJKName)
				.confConditionalRequired(ER3031PosLbFink2911.genConditional())));
		fs.add(sf(3, null, "Tanggal Pengalihan",
				sv(C, 1, 8, date).confConditionalRequired(ER3031PosLbFink2911.genConditional())));
		fs.add(sf(4, null, "Jumlah Borrower yang Dialihkan (Satuan Akun)", sv(M, 1, 16, numericNegatif)));
		fs.add(sf(5, null, "Nominal Pinjaman yang Dialihkan", sv(M, 1, 16, numericNegatif)));
		return res;
	}
}
