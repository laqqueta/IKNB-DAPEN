...
import id.go.ojk.fink.client.builder.field.lt.reference.ER6008PosLtFink2116;

public class Lt2116 extends BaseMetadata {

	public Lt2116(String reportCode) {
		super(reportCode, ".txt");
	}

	@Override
	public SubmissionFormat get() {
		EFormLaporanTahunan eEnum = EFormLaporanTahunan.LT_2116;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 1, null, ER6008PosLtFink2116.genFieldSave(), ER6008PosLtFink2116.genFieldSave(),
				ER6008PosLtFink2116.getRequiredPos());
		res.addSegmentValidations(ER6008PosLtFink2116.genRowValidation2());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 12, 12, refTable)
				.confReference(EHeaderMetadataLt.R6008_LT2116.getObject()).confRegex(patternNumeric)));
		fs.add(sf(2, null, "Nama Aset Tak Berwujud", sv(C, 1, 1000, all).confRegex(PATTERN_ALL_1)
				.confConditionalRequired(ER6008PosLtFink2116.genConditional())));
		...
		fs.add(sf(6, null, "Saldo Akhir", sv(M, 1, 16, numericNegatif))
				.addFieldValidations(ER6008PosLtFink2116.genFieldValidation6()));
