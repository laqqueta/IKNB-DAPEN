...
import id.go.ojk.pmvs.client.builder.field.rb.reference.ER2261PosRePmvs090203;
...
public class Re090203 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormRealisasiBisnis eEnum = EFormRealisasiBisnis.RE_090203;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 14, 14, refTable).confReference(EHeaderMetadataRb.R2261_RE090203.getObject())));
		...
		fs.add(sf(3, null, "Posisi Jabatan",
				sv(C, 0, 1000, all).confConditionalRequired(ER2261PosRePmvs090203.genConditionalField())));
		fs.add(sf(4, null, "Bidang Kerja Penugasan",
				sv(C, 0, 1000, all).confConditionalRequired(ER2261PosRePmvs090203.genConditionalField())));
		fs.add(sf(5, null, "Jangka Waktu",
				sv(C, 0, 255, all).confConditionalRequired(ER2261PosRePmvs090203.genConditionalField())));
		fs.add(sf(6, null, "Nama Tenaga Pendamping",
				sv(C, 0, 1000, all).confConditionalRequired(ER2261PosRePmvs090203.genConditionalField())));
		fs.add(sf(7, null, "Rencana Program Alih Pengetahuan",
				sv(C, 0, 1000, all).confConditionalRequired(ER2261PosRePmvs090203.genConditionalField())));
		fs.add(sf(8, null, "Realisasi Program Alih Pengetahuan",
				sv(C, 0, 1000, all).confConditionalRequired(ER2261PosRePmvs090203.genConditionalField())));
		fs.add(sf(9, null, "Alasan",
				sv(C, 0, 4000, all).confConditionalRequired(ER2261PosRePmvs090203.genConditionalField())));
		return res;
	}
}
