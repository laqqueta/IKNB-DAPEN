...
import id.go.ojk.pmvs.client.builder.field.rb.reference.ER2260PosRePmvs090202;
...
public class Re090202 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		fs.add(sf(3, null, "Metode Diklat", sv(C, 0, 10, refTable).confReference(EHeaderMetadataShared.R007.getObject())
				.confConditionalRequired(ER2260PosRePmvs090202.genConditionalField())));
		fs.add(sf(4, null, "Realisasi Waktu Diklat",
				sv(C, 0, 6, yearMonth).confConditionalRequired(ER2260PosRePmvs090202.genConditionalField())));
		fs.add(sf(5, null, "Durasi Diklat",
				sv(C, 0, 1000, all).confConditionalRequired(ER2260PosRePmvs090202.genConditionalField())));
		fs.add(sf(6, null, "Tempat",
				sv(C, 0, 1000, all).confConditionalRequired(ER2260PosRePmvs090202.genConditionalField())));
		fs.add(sf(7, null, "Narasumber/Lembaga Diklat",
				sv(C, 0, 1000, all).confConditionalRequired(ER2260PosRePmvs090202.genConditionalField())));
		fs.add(sf(8, null, "Jumlah Seluruh Peserta",
				sv(C, 0, 1000, numericNegatif).confConditionalRequired(ER2260PosRePmvs090202.genConditionalField())));
		fs.add(sf(9, null, "Rencana Biaya Diklat",
				sv(C, 0, 16, numericNegatif).confConditionalRequired(ER2260PosRePmvs090202.genConditionalField())));
		fs.add(sf(10, null, "Realisasi Biaya Diklat",
				sv(C, 0, 16, numericNegatif).confConditionalRequired(ER2260PosRePmvs090202.genConditionalField())));
		fs.add(sf(11, null, "Deviasi",
				sv(C, 0, 16, numericNegatif).confConditionalRequired(ER2260PosRePmvs090202.genConditionalField()))
				.addFieldValidations(ER2260PosRePmvs090202.genValidationField11()));
		return res;
	}
}
