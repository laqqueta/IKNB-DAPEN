...
import id.go.ojk.pmvs.client.builder.field.rb.reference.ER2012PosRbPmvs090204;
...
public class Rb090204 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormRencanaBisnis eEnum = EFormRencanaBisnis.RB_090204;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 14, 14, refTable).confReference(EHeaderMetadataRb.R2012_RB090204.getObject())));
		fs.add(sf(2, null, "Bidang Tugas",
				sv(O, 0, 1000, all).confConditionalRequired(ER2012PosRbPmvs090204.genConditionalField())));
		fs.add(sf(3, null, "Alasan Pemanfaatan Tenaga Alih Daya",
				sv(C, 0, 1000, all).confConditionalRequired(ER2012PosRbPmvs090204.genConditionalField())));
		fs.add(sf(4, null, "Jumlah Tenaga Kerja Alih Daya",
				sv(C, 1, 16, numeric).confConditionalRequired(ER2012PosRbPmvs090204.genConditionalField())));
