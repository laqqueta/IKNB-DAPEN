...
import id.go.ojk.bptk.client.builder.field.lbbptkjkp.reference.ER5712SBN;
...
public class LbSbn extends BaseMetadata {
  ...
  public SubmissionFormat get() {
    EFormLbbptkjkp eEnum = EFormLbbptkjkp.SBN;
    SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
        extension, 0, null);

    res.setSavePos(ER5712SBN.genFieldSave());

    res.addSegmentValidations(ER5712SBN.genRowValidation04());

    List<SubmissionField> fs = res.getFields();
    fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
    fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 13, 13, refTable)
        .confReference(EHeaderMetadataLbbptkjkp.R5712SBN.getObject())
