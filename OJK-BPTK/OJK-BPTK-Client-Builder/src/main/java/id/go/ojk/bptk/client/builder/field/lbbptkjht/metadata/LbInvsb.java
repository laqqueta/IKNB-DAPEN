...
import id.go.ojk.bptk.client.builder.field.lbbptkjht.reference.ER5608INVSB;
...
public class LbInvsb extends BaseMetadata {
  ...
  public SubmissionFormat get() {
    EFormLbbptkjht eEnum = EFormLbbptkjht.INVSB;
    SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
        extension, 0, null);

    res.setSavePos(ER5608INVSB.genFieldSave());

    List<SubmissionField> fs = res.getFields();
    fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
    fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 15, 15, refTable)
        .confReference(EHeaderMetadataLbbptkjht.R5608INVSB.getObject())
    ...
    fs.add(sf(16, null, "Suku Bunga atau Kupon, Tingkat Suku Bunga/Nisbah atau Kupon", sv(M, 1, 6, numericDot))
        .addFieldValidations(ER5608INVSB.genFieldValidation17()));
    ...
    fs.add(sf(29, null, "Hierarki Harga Pasar", sv(M, 1, 10, refTable)
