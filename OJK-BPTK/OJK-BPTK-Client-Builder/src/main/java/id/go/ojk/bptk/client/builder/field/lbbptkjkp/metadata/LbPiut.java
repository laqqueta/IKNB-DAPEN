...
import id.go.ojk.bptk.client.builder.field.lbbptkjkp.reference.ER5713PIUT;
...
public class LbPiut extends BaseMetadata {
  ...
  public SubmissionFormat get() {
    EFormLbbptkjkp eEnum = EFormLbbptkjkp.PIUT;
    SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
        extension, 0, null);

    res.setSavePos(ER5713PIUT.genFieldSave());

    /* -- ANTAR FORM -- */
    res.addSegmentValidations(ER5713PIUT.genRowValidation14());

    List<SubmissionField> fs = res.getFields();
    fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
    fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 14, 14, refTable)
        .confReference(EHeaderMetadataLbbptkjkp.R5713PIUT.getObject())
    ...
    fs.add(sf(4, null, "Jumlah Kontrak", sv(M, 1, 16, numeric))
        .addFieldValidations(ER5713PIUT.genFieldValidationJumlahKontrak()));
