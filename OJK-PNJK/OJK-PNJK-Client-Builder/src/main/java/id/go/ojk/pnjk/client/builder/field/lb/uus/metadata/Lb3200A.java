...
import id.go.ojk.pnjk.client.builder.field.lb.uus.reference.ER3126PosLbPnju3200A;
...
public class Lb3200A extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulananUus eEnum = EFormLaporanBulananUus.LB_3200A;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLbUus.R3126_LB3200A.getObject())));
		fs.add(sf(2, null, "Lokasi Terjamin (Makfuul 'Anhu) - (Dati II)", sv(M, 1, 10, refTable)
		...
		fs.add(sf(15, null, "Kafalah KUR - Non Mikro-Jumlah Terjamin (Makfuul 'Anhu) - (Satuan)",
				sv(M, 1, 8, numeric)));
		...
		fs.add(sf(18, null, "Total-Jumlah Terjamin (Makfuul 'Anhu) - (Satuan)", sv(M, 1, 8, numeric))
				.addFieldValidations(ER3126PosLbPnju3200A.genFieldValidation18()));
		fs.add(sf(19, null, "Total-Nilai Kafalah (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3126PosLbPnju3200A.genFieldValidation19()));
		fs.add(sf(20, null, "Total-Nilai Klaim (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3126PosLbPnju3200A.genFieldValidation20()));
		return res;
	}
}
