...
import id.go.ojk.pnjk.client.builder.field.lb.uus.reference.ER3117PosLbPnju2300;
...
public class Lb2300 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null, null, ER3117PosLbPnju2300.genFieldSave(), null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLbUus.R3117_LB2300.getObject())));
		fs.add(sf(2, null, "Nama", sv(M, 0, 100, all)));
		fs.add(sf(3, null, "Nomor Sertifikat Penjaminan Kafalah", sv(M, 1, 50, all)));
		fs.add(sf(4, null, "Tanggal Sertifikat Kafalah", sv(M, 8, 8, date)));
		fs.add(sf(5, null, "Jenis Kafalah", sv(M, 1, 10, refTable).confRegex(PATTERN_REFERENCE_1)
				.confReference(EHeaderMetadataShared.R042.getObject())));
		fs.add(sf(6, null, "Produk Kafalah", sv(M, 1, 10, refTable).confRegex(PATTERN_REFERENCE_1)
				.confReference(EHeaderMetadataShared.R043.getObject())));
		fs.add(sf(7, null, "Nilai Pembiayaan/Proyek", sv(M, 1, 16, numeric)));
		fs.add(sf(8, null, "Nilai Kafalah", sv(M, 1, 16, numeric)));
		fs.add(sf(9, null, "Nilai Kafalah Porsi Mitra", sv(M, 1, 16, numeric)));
