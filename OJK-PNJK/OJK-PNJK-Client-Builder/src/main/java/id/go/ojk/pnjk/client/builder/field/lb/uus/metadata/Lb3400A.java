...
import id.go.ojk.pnjk.client.builder.field.lb.uus.reference.ER3130PosLbPnju3400A;
...
public class Lb3400A extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulananUus eEnum = EFormLaporanBulananUus.LB_3400A;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLbUus.R3130_LB3400A.getObject())));
		...
		fs.add(sf(18, null, "Total-Jumlah Terjamin (Makfuul 'Anhu)-(Satuan)", sv(M, 1, 8, numeric))
				.addFieldValidations(ER3130PosLbPnju3400A.genFieldValidation18()));
		fs.add(sf(19, null, "Total-Nilai Kafalah (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3130PosLbPnju3400A.genFieldValidation19()));
		fs.add(sf(20, null, "Total-Nilai Klaim (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3130PosLbPnju3400A.genFieldValidation20()));
		return res;
	}
}
