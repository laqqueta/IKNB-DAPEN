...
import id.go.ojk.pnjk.client.builder.field.lb.uus.reference.ER3129PosLbPnju3300B;
...
public class Lb3300B extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulananUus eEnum = EFormLaporanBulananUus.LB_3300B;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLbUus.R3129_LB3300B.getObject())));
		fs.add(sf(2, null, "Lokasi Terjamin (Makfuul Lahu)-(Dati II)", sv(M, 1, 10, refTable)
				.confRegex(PATTERN_REFERENCE_1).confReference(EHeaderMetadataShared.R033.getObject())));
		fs.add(sf(3, null, "Kafalah atas Surat Utang-Jumlah Terjamin (Makfuul Lahu)-(Satuan)", sv(M, 1, 8, numeric)));
		...
		fs.add(sf(6, null, "Kafalah Pembelian Barang secara Angsuran-Jumlah Terjamin (Makfuul Lahu)-(Satuan)",
				sv(M, 1, 8, numeric)));
		...
		fs.add(sf(9, null, "Kafalah Transaksi Dagang-Jumlah Terjamin (Makfuul Lahu)-(Satuan)", sv(M, 1, 8, numeric)));
		...
		fs.add(sf(12, null,
				"Kafalah Pengadaan barang dan/atau Jasa (Surety Bond)-Jumlah Terjamin (Makfuul Lahu)-(Satuan)",
				sv(M, 1, 8, numeric)));
		...
		fs.add(sf(14, null, "Kafalah Pengadaan barang dan/atau Jasa (Surety Bond)-Nilai Klaim (Rp)",
				sv(M, 1, 16, numeric)));
		fs.add(sf(15, null, "Kafalah Bank Garansi (Kontra Bank Garansi)-Jumlah Terjamin (Makfuul Lahu)-(Satuan)",
				sv(M, 1, 8, numeric)));
		...
		fs.add(sf(18, null, "Kafalah Surat Kredit Berdokumen dalam Negeri-Jumlah Terjamin (Makfuul Lahu)-(Satuan)",
				sv(M, 1, 8, numeric)));
		...
		fs.add(sf(21, null, "Kafalah Letter of Credit-Jumlah Terjamin (Makfuul Lahu)-(Satuan)", sv(M, 1, 8, numeric)));
		...
		fs.add(sf(24, null, "Kafalah Kepabeanan (Customs Bond)-Jumlah Terjamin (Makfuul Lahu)-(Satuan)",
				sv(M, 1, 8, numeric)));
		...
		fs.add(sf(27, null, "Kafalah Cukai-Jumlah Terjamin (Makfuul Lahu)-(Satuan)", sv(M, 1, 8, numeric)));
		...
		fs.add(sf(30, null,
				"Pemberian Jasa Konsultasi Manajemen terkait dengan Kegiatan Usaha Penjamin-Jumlah Terjamin (Makfuul Lahu)-(Satuan)",
				sv(M, 1, 8, numeric)));
		fs.add(sf(31, null,
		...
		fs.add(sf(32, null,
				...
				sv(M, 1, 16, numeric)));
		fs.add(sf(33, null,
				"Kegiatan Usaha Lainnya setelah Mendapat Persetujuan dari OJK-Jumlah Terjamin (Makfuul Lahu)-(Satuan)",
				sv(M, 1, 8, numeric)));
		...
		fs.add(sf(35, null, "Kegiatan Usaha Lainnya setelah Mendapat Persetujuan dari OJK-Nilai Klaim (Rp)",
				sv(M, 1, 16, numeric)));
		fs.add(sf(36, null, "Total-Jumlah Terjamin (Makfuul Lahu)-(Satuan)", sv(M, 1, 8, numeric))
				.addFieldValidations(ER3129PosLbPnju3300B.genFieldValidation36()));
		fs.add(sf(37, null, "Total-Nilai Kafalah (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3129PosLbPnju3300B.genFieldValidation37()));
		fs.add(sf(38, null, "Total-Nilai Klaim (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3129PosLbPnju3300B.genFieldValidation38()));
		return res;
	}
}
