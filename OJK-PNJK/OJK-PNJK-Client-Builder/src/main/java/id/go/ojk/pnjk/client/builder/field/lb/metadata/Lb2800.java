...
import id.go.ojk.pnjk.client.builder.field.lb.reference.ER3022PosLbPnjk2800;
...
public class Lb2800 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulanan eEnum = EFormLaporanBulanan.LB_2800;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null, null, ER3022PosLbPnjk2800.genFieldSave(), null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLb.R3022_LB2800.getObject())));
		...
		fs.add(sf(6, null, "Nomor Sertifikat Penjaminan", sv(M, 1, 50, all)));
		fs.add(sf(7, null, "Tanggal Sertifikat Penjaminan", sv(M, 8, 8, date)));
		fs.add(sf(8, null, "Jenis Penerima Jaminan", sv(M, 1, 10, refTable).confRegex(PATTERN_REFERENCE_1)
				.confReference(EHeaderMetadataShared.R046.getObject())));
		fs.add(sf(9, null, "Nama Penerima Jaminan", sv(M, 1, 100, all)));
		fs.add(sf(10, null, "Jenis Penjaminan", sv(M, 1, 10, refTable).confRegex(PATTERN_REFERENCE_1)
				.confReference(EHeaderMetadataShared.R042.getObject())));
		fs.add(sf(11, null, "Produk Penjaminan", sv(M, 1, 10, refTable).confRegex(PATTERN_REFERENCE_1)
		...
		fs.add(sf(15, null, "Nilai Kredit/Proyek", sv(M, 1, 16, numeric)));
		fs.add(sf(16, null, "Nilai Penjaminan", sv(M, 1, 16, numeric)));
		fs.add(sf(17, null, "Nilai Co-Guarantee/Reasuransi", sv(M, 1, 16, numeric)));
		fs.add(sf(18, null, "Outstanding Penjaminan Retensi Sendiri", sv(M, 1, 16, numeric)));
		fs.add(sf(19, null, "Tanggal Awal Penjaminan", sv(M, 8, 8, date)));
		fs.add(sf(20, null, "Tanggal Akhir Penjaminan", sv(M, 8, 8, date)));
		fs.add(sf(21, null, "Jumlah Hari Utang IJP", sv(M, 1, 8, numeric)));
		fs.add(sf(22, null, "Nilai IJP", sv(M, 1, 16, numeric)));
		fs.add(sf(23, null, "Utang IJP", sv(M, 1, 16, numeric)));
