...
import id.go.ojk.pnjk.client.builder.field.lb.uus.reference.ER3127PosLbPnju3200B;
import id.go.ojk.pnjk.client.builder.field.reference.EHeaderMetadataShared;

public class Lb3200B extends BaseMetadata {

	public Lb3200B(String reportCode) {
		super(reportCode, ".txt");
	}

	@Override
	public SubmissionFormat get() {
		EFormLaporanBulananUus eEnum = EFormLaporanBulananUus.LB_3200B;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLbUus.R3127_LB3200B.getObject())));
		...
		fs.add(sf(36, null, "Total-Jumlah Terjamin (Makfuul 'Anhu)-(Satuan)", sv(M, 1, 8, numeric))
				.addFieldValidations(ER3127PosLbPnju3200B.genFieldValidation36()));
		fs.add(sf(37, null, "Total-Nilai Kafalah (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3127PosLbPnju3200B.genFieldValidation37()));
		fs.add(sf(38, null, "Total-Nilai Klaim (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3127PosLbPnju3200B.genFieldValidation38()));
		return res;
	}
}
