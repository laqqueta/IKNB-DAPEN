...
import id.go.ojk.pnjk.client.builder.field.lb.reference.ER3031PosLbPnjk3400B;
...
public class Lb3400B extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulanan eEnum = EFormLaporanBulanan.LB_3400B;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLb.R3031_LB3400B.getObject())));
		...
		fs.add(sf(36, null, "Total-Jumlah Terjamin (Satuan)", sv(M, 1, 8, numeric))
				.addFieldValidations(ER3031PosLbPnjk3400B.genFieldValidation36()));
		fs.add(sf(37, null, "Total-Nilai Jaminan (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3031PosLbPnjk3400B.genFieldValidation37()));
		fs.add(sf(38, null, "Total-Nilai Klaim (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3031PosLbPnjk3400B.genFieldValidation38()));
		return res;
	}
}
