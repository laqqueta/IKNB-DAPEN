...
import id.go.ojk.pnjk.client.builder.field.lb.uus.reference.ER3132PosLbPnju3500A;
...
public class Lb3500A extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulananUus eEnum = EFormLaporanBulananUus.LB_3500A;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 4, 4, ER3132PosLbPnju3500A.genFieldSave(), ER3132PosLbPnju3500A.genFieldSave(),
				ER3132PosLbPnju3500A.getRequiredPos());
		res.addSegmentValidations(ER3132PosLbPnju3500A.genValidation5());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLbUus.R3132_LB3500A.getObject())).confUnique(U));
		...
		fs.add(sf(32, null, "Total-Pengajuan Klaim (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3132PosLbPnju3500A.genFieldValidation32()));
		fs.add(sf(33, null, "Total-Klaim Diproses (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3132PosLbPnju3500A.genFieldValidation33()));
		fs.add(sf(34, null, "Total-Klaim Disetujui (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3132PosLbPnju3500A.genFieldValidation34()));
		fs.add(sf(35, null, "Total-Klaim Dibayar (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3132PosLbPnju3500A.genFieldValidation35()));
		fs.add(sf(36, null, "Total-Klaim Ditolak (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3132PosLbPnju3500A.genFieldValidation36()));
		fs.add(sf(37, null, "Total-Klaim Disetujui Namun Belum Dibayar (Rp)", sv(M, 1, 16, numeric))
				.addFieldValidations(ER3132PosLbPnju3500A.genFieldValidation37()));
		return res;
	}
}
