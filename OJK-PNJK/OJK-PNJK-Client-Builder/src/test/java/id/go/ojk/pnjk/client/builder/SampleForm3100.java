...
public class SampleForm3100 extends BaseSampleForm {
	...
	public SampleForm3100(String folder, String periode, String ljk, String kodeKoreksi, int dataCount) {
		super(folder, periode, ljk, kodeKoreksi, dataCount);
	}

	@Override
	public String genFilename() {
		String filename = "LBPPNJK-3100-{0}-M-{1}-{2}-01.txt";
		return genFilename(filename);
	}

	@Override
	public void writeHeader(BufferedWriter writer) throws IOException {
		final String[] HEADER = new String[] { "H01", "030403", "{ljk}", "{periode}", "LBPPNJK", "3100",
				"{kodekoreksi}", "" };
	...
	public void writeDetail(BufferedWriter writer) throws IOException {
		...
		List<KeyValueString> jenisPenerimaJaminan = ER1189JenisPenerimaJaminan.getObjects();
	...
	private String getTanggal(String[] tanggal, Random random) {
		int idx = getRandomInt(random, 0, tanggal.length - 1);
		return tanggal[idx];
	}
}
