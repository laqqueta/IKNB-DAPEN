...
public class SampleForm3300BUus extends BaseSampleForm {

	public SampleForm3300BUus(String folder, String periode, String ljk, String kodeKoreksi, int dataCount) {
		super(folder, periode, ljk, kodeKoreksi, dataCount);
	}

	@Override
	public String genFilename() {
		String filename = "LBPPNJU-3300B-{0}-M-{1}-{2}-03.txt";
		return genFilename(filename);
	}

	@Override
	public void writeHeader(BufferedWriter writer) throws IOException {
		final String[] HEADER = new String[] { "H01", "030403", "{ljk}", "{periode}", "LBPPNJU", "3300B",
				"{kodekoreksi}", "" };
