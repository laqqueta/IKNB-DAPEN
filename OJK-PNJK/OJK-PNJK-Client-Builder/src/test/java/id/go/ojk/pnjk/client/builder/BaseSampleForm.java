...
public abstract class BaseSampleForm implements ISampleForm {
	...
	protected int getRandomInt(Random random, int min, int max) {
		return random.nextInt(max - min) + min;
	}
	
	protected String getRandomAlphaNumeric(Random random, int min, int max) {
		int len = getRandomInt(random, min, max);
		return RandomStringUtils.random(len, true, true);
	}
	
	protected String getRandomNumeric(Random random, int min, int max) {
		int len = getRandomInt(random, min, max);
