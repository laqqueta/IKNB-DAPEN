...
import id.go.ojk.ppk.client.builder.field.rb.reference.ER2253PosRePpk040301;
...
public class Re040301 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 3, null, ER2253PosRePpk040301.genFieldSave(), null, ER2253PosRePpk040301.getRequiredPos());
		res.addSegmentValidations(ER2253PosRePpk040301.genValidationNumeric());
		res.addSegmentValidations(ER2253PosRePpk040301.genValidationNumericDot());
		res.addSegmentValidations(ER2253PosRePpk040301.genValidationTotalPenyaluranPembiayaankepadaPihakTerkait());
		res.addSegmentValidations(
				ER2253PosRePpk040301.genValidationRasioPenyaluranPembiayaanPihakTerkaitTerhadapEkuitas());
		...
		fs.add(sf(2, null, "Nama Debitur Pihak Terkait",
				sv(C, 0, 255, alfaSpace).confConditionalRequired(ER2253PosRePpk040301.genConditionalFieldMustEmpty())));
		fs.add(sf(3, null, "Jenis Pembiayaan",
				sv(C, 0, 6, refTable).confReference(EHeaderMetadataShared.R001.getObject())
						.confConditionalRequired(ER2253PosRePpk040301.genConditionalField1())));
		...
		fs.add(sf(5, null, "Proyeksi",
				sv(C, 0, 16, all).confConditionalRequired(ER2253PosRePpk040301.genConditionalField2())));
		fs.add(sf(6, null, "Realisasi",
				sv(C, 0, 16, all).confConditionalRequired(ER2253PosRePpk040301.genConditionalField2())));
		fs.add(sf(7, null, "Deviasi",
				sv(C, 0, 16, all).confConditionalRequired(ER2253PosRePpk040301.genConditionalField2()))
				.addFieldValidations(ER2253PosRePpk040301.genValidationSum()));
		return res;
	}
}
