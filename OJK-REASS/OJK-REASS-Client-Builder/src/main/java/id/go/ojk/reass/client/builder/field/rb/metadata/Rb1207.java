...
import id.go.ojk.reass.client.builder.field.rb.reference.ER2025PosRbReass1207;
...
public class Rb1207 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 7, null, null, null, ER2025PosRbReass1207.getRequiredPos());
		res.addSegmentValidations(ER2025PosRbReass1207.genValidationNumeric());
		res.addSegmentValidations(ER2025PosRbReass1207.genValidationNumericDot());
		res.addSegmentValidations(ER2025PosRbReass1207.genValidationNumericMaxLength());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 14, 14, refTable).confReference(EHeaderMetadataRb.R2025_RB1207.getObject())));
		fs.add(sf(2, null, "Asumsi Lain",
				sv(C, 0, 200, all).confConditionalRequired(ER2025PosRbReass1207.genConditionField())));
