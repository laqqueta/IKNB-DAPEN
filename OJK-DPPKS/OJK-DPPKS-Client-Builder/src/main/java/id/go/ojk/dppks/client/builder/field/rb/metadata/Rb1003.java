...
import id.go.ojk.dppks.client.builder.field.rb.reference.ER2014PosRbDppks1003;
...
public class Rb1003 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		fs.add(sf(3, null, "Nama Peserta",
				sv(C, 1, 150, all).confConditionalRequired(ER2014PosRbDppks1003.genConditionalAllExist())));
		fs.add(sf(4, null, "Jabatan",
				sv(C, 1, 150, all).confConditionalRequired(ER2014PosRbDppks1003.genConditionalAllExist())));
		fs.add(sf(5, null, "Sertifikasi",
				sv(C, 1, 100, all).confConditionalRequired(ER2014PosRbDppks1003.genConditionalAllExist())));
		fs.add(sf(6, null, "Waktu Pelatihan",
				sv(C, 1, 50, all).confConditionalRequired(ER2014PosRbDppks1003.genConditionalAllExist())));
		return res;
	}
}
