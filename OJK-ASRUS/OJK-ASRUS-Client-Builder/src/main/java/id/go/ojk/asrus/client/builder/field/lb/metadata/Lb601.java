...
import id.go.ojk.asrus.client.builder.field.lb.reference.ER3013PosLbAsrus601;
...
public class Lb601 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode,
				new ArrayList<>(), extension, 6, 6, ER3013PosLbAsrus601.genFieldSave(), null,
				ER3013PosLbAsrus601.getRequiredPos());
		res.addSegmentValidations(ER3013PosLbAsrus601.genValidationJumlahLiabilitas());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLb.R3013_LB601.getObject())).confUnique(U));
		...
		fs.add(sf(4, null, "Selisih Penilaian SAK dan SAP", sv(M, 1, 16, numericNegatif))
				.addFieldValidations(ER3013PosLbAsrus601.genValidation4()));
		return res;
	}
}
