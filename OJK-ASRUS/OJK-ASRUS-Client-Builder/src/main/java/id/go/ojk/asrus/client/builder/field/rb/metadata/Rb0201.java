...
import id.go.ojk.asrus.client.builder.field.rb.reference.ER2006PosRbAsrus0201;
...
public class Rb0201 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode,
				...
				ER2006PosRbAsrus0201.getRequiredPos());
		res.addSegmentValidations(ER2006PosRbAsrus0201.genValidationNumeric());
		res.addSegmentValidations(ER2006PosRbAsrus0201.genValidationNumericDot());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataRb.R2006_RB0201.getObject())));
		fs.add(sf(2, null, "Indikator Lain",
				sv(C, 0, 200, all).confConditionalRequired(ER2006PosRbAsrus0201.genConditionalField2())));
		...
		fs.add(sf(5, null, "%", sv(M, 0, 16, numericDotNegatif))
				.addFieldValidations(ER2006PosRbAsrus0201.genValidationRatioField()));
		return res;
	}
}
