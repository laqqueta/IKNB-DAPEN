...
import id.go.ojk.asrjk.client.builder.field.setup.EMessage;
...
public class ReferenceBuilder extends BaseReferenceBuilder {
	...
	private Map<String, List<KeyValueString>> getReferences() {
		...
		res.putAll(EHeaderMetadataPls.getMap());
		res.putAll(EHeaderMetadataPlsUus.getMap());
	...
	private List<ReferenceMetadata> getReferenceMetadata() {
		...
		res.addAll(EHeaderMetadataPls.getObjects());
		res.addAll(EHeaderMetadataPlsUus.getObjects());
