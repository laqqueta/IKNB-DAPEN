...
import id.go.ojk.asrjk.client.builder.field.rb.uus.reference.ER2119PosRbAsrju1001;
...
public class Rb1001 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode,
				new ArrayList<>(), extension, 1, null, ER2119PosRbAsrju1001.genFieldSave(), null,
				ER2119PosRbAsrju1001.getRequiredPos());
		res.addSegmentValidations(ER2119PosRbAsrju1001.genValidationTotal());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataRbUus.R2119_RB1001.getObject())));
		fs.add(sf(2, null, "Level Jabatan",
				sv(C, 1, 1000, all).confConditionalRequired(ER2119PosRbAsrju1001.genConditionalMustEmpty())));
		fs.add(sf(3, null, "Jumlah SDM per 30 Sept 20XX-1",
				sv(C, 1, 16, numeric).confConditionalRequired(ER2119PosRbAsrju1001.genConditionalField())));
