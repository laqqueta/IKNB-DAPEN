...
import id.go.ojk.asrjk.client.builder.field.pls.uus.reference.ER6101PosPlsAsrju0100;
...
public class Pls0100 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanPolisUus eEnum = EFormLaporanPolisUus.PLS_0100;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 2, null, ER6101PosPlsAsrju0100.genFieldSave(), null);
		res.setRequiredPos(ER6101PosPlsAsrju0100.getRequiredPos());
		res.setUniquePos(ER6101PosPlsAsrju0100.getUniquePos());
		res.setSavePosFormDb(ER6101PosPlsAsrju0100.genFieldSaveForm());
		res.addSegmentValidations(ER6101PosPlsAsrju0100.genValidationTotal());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataPlsUus.R6101_PLS001.getObject())));
		fs.add(sf(2, null, "Nomor Polis", sv(C, 1, 50, all).confRegex(PATTERN_NAME_5)
				.confConditionalRequired(ER6101PosPlsAsrju0100.genConditionalExistPosAndComparatorHasValueA())));
		fs.add(sf(3, null, "Nama Pemegang Polis", sv(C, 1, 255, all).confRegex(PATTERN_NAME_6)
				.confConditionalRequired(ER6101PosPlsAsrju0100.genConditionalExistPosAndComparatorNotEmptyA())));
		fs.add(sf(4, null, "Jenis Polis",
				sv(C, 3, 3, refTable).confRegex(patternNumeric).confReference(EHeaderMetadataShared.R047.getObject())
						.confConditionalRequired(ER6101PosPlsAsrju0100.genConditionalExistPos())));
		fs.add(sf(5, null, "Tanggal Mulai Polis",
				sv(C, 8, 8, date)
						.confConditionalRequired(ER6101PosPlsAsrju0100.genConditionalExistPosAndComparatorHasValueC()))
				.addFieldValidations(ER6101PosPlsAsrju0100.genValidationPeriodeLaporan()));
		fs.add(sf(6, null, "Tanggal Berakhir Polis",
				sv(C, 8, 8, date)
						.confConditionalRequired(ER6101PosPlsAsrju0100.genConditionalExistPosAndComparatorHasValueC()))
				.addFieldValidations(ER6101PosPlsAsrju0100.genValidationField06()));
		fs.add(sf(7, null, "NIK/NPWP",
				sv(C, 1, 22, alfaNumeric)
						.confConditionalRequired(ER6101PosPlsAsrju0100.genConditionalExistPosAndComparatorNotEmptyA()))
				.addFieldValidations(ER6101PosPlsAsrju0100.genValidationNIK())
				.addFieldValidations(ER6101PosPlsAsrju0100.genValidationNIK2()));
		fs.add(sf(8, null, "Tanggal Lahir Pemegang Polis", sv(C, 8, 8, date)
				.confConditionalRequired(ER6101PosPlsAsrju0100.genConditionalExistPosAndComparatorHasValueB())));
		fs.add(sf(9, null, "Jenis Kelamin",
				sv(C, 1, 1, refTable).confRegex(patternAlfa).confReference(EHeaderMetadataShared.R048.getObject())
						.confConditionalRequired(
								ER6101PosPlsAsrju0100.genConditionalExistPosAndComparatorHasValueB())));
		fs.add(sf(10, null, "Lokasi Pemegang Polis",
				sv(C, 4, 4, refTable).confRegex(patternNumeric).confReference(EHeaderMetadataShared.R049.getObject())
						.confConditionalRequired(
								ER6101PosPlsAsrju0100.genConditionalExistPosAndComparatorNotEmptyA())));
		...
		fs.add(sf(13, null, "Jumlah Kontribusi - Ujrah", sv(C, 1, 20, numericNegatif)
				.confConditionalRequired(ER6101PosPlsAsrju0100.genConditionalExistPosAndComparatorNotEmptyB())));
		...
		fs.add(sf(26, null, "Jumlah tertanggung",
				sv(C, 1, 12, numeric)
						.confConditionalRequired(ER6101PosPlsAsrju0100.genConditionalExistPosAndComparatorHasValueD()))
				.addFieldValidations(ER6101PosPlsAsrju0100.genValidationField26A())
				.addFieldValidations(ER6101PosPlsAsrju0100.genValidationField26B()));
		return res;
	}
}
