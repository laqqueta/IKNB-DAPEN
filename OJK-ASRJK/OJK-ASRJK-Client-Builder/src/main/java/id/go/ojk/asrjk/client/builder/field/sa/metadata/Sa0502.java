...
import id.go.ojk.asrjk.client.builder.field.sa.reference.ER4015PosSaAsrjk0502;
import id.go.ojk.client.model.config.SubmissionField;
import id.go.ojk.client.model.config.SubmissionFormat;
import id.go.ojk.conf.client.BaseMetadata;

public class Sa0502 extends BaseMetadata {

	public Sa0502(String reportCode) {
		super(reportCode, ".txt");
	}

	@Override
	public SubmissionFormat get() {
		EFormSelfAssessment eEnum = EFormSelfAssessment.SA_0502;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode,
				new ArrayList<>(), extension, 10, 10, null, null, ER4015PosSaAsrjk0502.getRequiredPos());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataSa.R4015_SA0502.getObject())).confUnique(U));
		fs.add(sf(2, null, "Individu - Peringkat Risiko Inheren", sv(M, 1, 1, numeric))
				.addFieldValidations(ER4015PosSaAsrjk0502.genValidationPeringkat()));
