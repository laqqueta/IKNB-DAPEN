...
import id.go.ojk.dppkk.client.builder.field.sa.reference.ER4003PosSaDppk0302;
...
public class Sa0302 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 9, 9, null, null, ER4003PosSaDppk0302.getRequiredPos());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataSa.R4003_SA0302.getObject())).confUnique(U));
		fs.add(sf(2, null, "Individu - Analisis Risiko Inheren", sv(M, 1, 4000, all)));
		fs.add(sf(3, null, "Individu - Analisis Kualitas Penerapan Manajemen Risiko", sv(M, 1, 4000, all)));
		fs.add(sf(4, null, "Individu - Analisis Tingkat Risiko", sv(M, 1, 4000, all)));
		return res;
	}
}
