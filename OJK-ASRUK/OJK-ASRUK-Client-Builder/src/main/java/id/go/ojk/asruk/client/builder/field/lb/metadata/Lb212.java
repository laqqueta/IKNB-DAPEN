...
import id.go.ojk.asruk.client.builder.field.lb.reference.ER3048PosLbAsruk212;
...
public class Lb212 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode,
				new ArrayList<>(), extension, 24, 24, ER3048PosLbAsruk212.genFieldSave(), null,
				ER3048PosLbAsruk212.getRequiredPos());
		res.addSegmentValidations(ER3048PosLbAsruk212.genValidationTotal());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLb.R3048_LB212.getObject())).confUnique(U));
		fs.add(sf(2, null, "Premi (Rp)", sv(M, 1, 16, numericNegatif)));
		fs.add(sf(3, null, "Jumlah Polis", sv(M, 1, 16, numericNegatif)));
		fs.add(sf(4, null, "Klaim (Rp)", sv(M, 1, 16, numericNegatif)));
