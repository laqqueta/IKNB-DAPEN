...
import id.go.ojk.asruk.client.builder.field.pls.uus.reference.ER6102PosPlsAsruu0200;
...
public class Pls0200 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 1, null, ER6102PosPlsAsruu0200.genFieldSave(), null, ER6102PosPlsAsruu0200.getRequiredPos());
		res.setSavePosFormDb(ER6102PosPlsAsruu0200.genFieldSaveForm());
		res.addSegmentValidations(ER6102PosPlsAsruu0200.genValidation2());
		res.addSegmentValidations(ER6102PosPlsAsruu0200.genValidation3());
		res.addSegmentValidations(ER6102PosPlsAsruu0200.genValidation4());
		...
		fs.add(sf(2, null, "Status Pertanggungan",
				sv(C, 3, 3, refTable).confRegex(patternNumeric).confReference(EHeaderMetadataShared.R066.getObject())
						.confConditionalRequired(ER6102PosPlsAsruu0200.genConditionalExistPos())));
		fs.add(sf(3, null, "Nomor Polis", sv(C, 1, 50, all).confRegex(PATTERN_NAME_5)
				.confConditionalRequired(ER6102PosPlsAsruu0200.genConditionalExistPosAndComparatorNotEmptyA())));
		fs.add(sf(4, null, "Nama Pemegang Polis", sv(C, 1, 255, all).confRegex(PATTERN_NAME_6)
				.confConditionalRequired(ER6102PosPlsAsruu0200.genConditionalExistPosAndComparatorNotEmptyB())));
		fs.add(sf(5, null, "NIK/NPWP",
				sv(C, 1, 22, alfaNumeric)
						.confConditionalRequired(ER6102PosPlsAsruu0200.genConditionalExistPosAndComparatorNotEmptyB()))
				.addFieldValidations(ER6102PosPlsAsruu0200.genValidationNIK())
				.addFieldValidations(ER6102PosPlsAsruu0200.genValidationNIK3()));
		fs.add(sf(6, null, "Lini Usaha",
				sv(C, 3, 4, refTable).confRegex(patternNumeric).confReference(EHeaderMetadataShared.R064.getObject())
						.confConditionalRequired(
								ER6102PosPlsAsruu0200.genConditionalExistPosAndComparatorNotEmptyB())));
		fs.add(sf(7, null, "Jenis Klaim",
				sv(C, 3, 3, refTable).confRegex(patternNumeric).confReference(EHeaderMetadataShared.R068.getObject())
						.confConditionalRequired(
								ER6102PosPlsAsruu0200.genConditionalExistPosAndComparatorHasValueB())));
		fs.add(sf(8, null, "Nilai Klaim Disetujui - Dana Tabarru'",
				sv(C, 1, 20, numericNegatif)
						.confConditionalRequired(ER6102PosPlsAsruu0200.genConditionalExistPosAndComparatorHasValueA())));
