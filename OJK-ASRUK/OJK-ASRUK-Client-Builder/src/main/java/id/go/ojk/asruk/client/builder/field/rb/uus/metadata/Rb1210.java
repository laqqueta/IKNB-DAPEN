...
import id.go.ojk.asruk.client.builder.field.rb.uus.reference.ER2133PosRbAsruu1210;
...
public class Rb1210 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormRencanaBisnisUus eEnum = EFormRencanaBisnisUus.RB_1210;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode,
				new ArrayList<>(), extension, 7, null, null, null,
				ER2133PosRbAsruu1210.getRequiredPos());
		res.addSegmentValidations(ER2133PosRbAsruu1210.genValidationNumericNegative());
		res.addSegmentValidations(ER2133PosRbAsruu1210.genValidationNumericDotNegative());
		res.addSegmentValidations(ER2133PosRbAsruu1210.genValidationNumericMaxLength());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 14, 14, refTable).confReference(EHeaderMetadataRbUus.R2133_RB1210.getObject())));
		fs.add(sf(2, null, "Asumsi Lain",
				sv(C, 0, 200, all).confConditionalRequired(ER2133PosRbAsruu1210.genConditionField())));
