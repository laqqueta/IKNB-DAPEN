...
import id.go.ojk.asruk.client.builder.field.rb.uus.reference.ER2112PosRbAsruu0602;
...
public class Rb0602 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormRencanaBisnisUus eEnum = EFormRencanaBisnisUus.RB_0602;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode,
				new ArrayList<>(), extension, 0, null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataRbUus.R2112_RB0602.getObject())));
		...
		fs.add(sf(3, null, "Aktual per 30 Sept 20XX-1", sv(C, 0, 16, numericNegatif)
				.confConditionalRequired(ER2112PosRbAsruu0602.genConditionalField())));
