...
import id.go.ojk.asruk.client.builder.field.lb.reference.ER3059PosLbAsruk620;
...
public class Lb620 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		fs.add(sf(3, null, "Mata Uang", sv(C, 1, 6, refTable).confReference(EHeaderMetadataShared.R029.getObject())
				.confConditionalRequired(ER3059PosLbAsruk620.genConditionField())));
		fs.add(sf(4, null, "Kurs",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3059PosLbAsruk620.genConditionField())));
		fs.add(sf(5, null, "NAB",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3059PosLbAsruk620.genConditionField())));
		fs.add(sf(6, null, "NAB/Unit",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3059PosLbAsruk620.genConditionField())));
		fs.add(sf(7, null, "Hasil Investasi Tahunan",
				sv(C, 1, 16, numericDot).confConditionalRequired(ER3059PosLbAsruk620.genConditionField())));
