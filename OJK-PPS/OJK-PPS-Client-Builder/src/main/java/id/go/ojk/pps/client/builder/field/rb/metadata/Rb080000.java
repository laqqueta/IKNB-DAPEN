...
import id.go.ojk.pps.client.builder.field.rb.reference.ER2013PosRbPps080000;
...
public class Rb080000 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null, ER2013PosRbPps080000.genFieldSave());
		res.addSegmentValidations(ER2013PosRbPps080000.genValidationUnique());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 14, 14, refTable).confReference(EHeaderMetadataRb.R2013_RB080000.getObject())));
		...
		fs.add(sf(3, null, "Jenis Kantor",
				sv(C, 0, 5, refTable).confReference(EHeaderMetadataShared.R005.getObject())
						.confConditionalRequired(ER2013PosRbPps080000.genConditionalField3()))
				.addFieldValidations(ER2013PosRbPps080000.genValidationField3()));
		fs.add(sf(4, null, "Waktu Pelaksanaan",
				sv(C, 0, 8, date).confConditionalRequired(ER2013PosRbPps080000.genConditionalField4())));
		fs.add(sf(5, null, "Perkiraan Investasi/Biaya",
				sv(C, 0, 16, numericNegatif).confConditionalRequired(ER2013PosRbPps080000.genConditionalField5())));
		fs.add(sf(6, null, "Lokasi", sv(C, 0, 8, refTable).confReference(EHeaderMetadataShared.R006.getObject())
				.confConditionalRequired(ER2013PosRbPps080000.genConditionalField4())));
		fs.add(sf(7, null, "Keterangan",
				sv(C, 0, 1000, all).confConditionalRequired(ER2013PosRbPps080000.genConditionalField7())));
		return res;
	}
}
