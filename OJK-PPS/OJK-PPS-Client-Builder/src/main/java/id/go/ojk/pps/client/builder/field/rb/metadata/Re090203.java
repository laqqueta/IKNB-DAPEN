...
import id.go.ojk.pps.client.builder.field.rb.reference.ER2265PosRePps090203;
...
public class Re090203 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 14, 14, refTable).confReference(EHeaderMetadataRb.R2265_RE090203.getObject())));
		...
		fs.add(sf(3, null, "Posisi Jabatan",
				sv(C, 0, 1000, all).confConditionalRequired(ER2265PosRePps090203.genConditionalField())));
		fs.add(sf(4, null, "Bidang Kerja Penugasan",
				sv(C, 0, 1000, all).confConditionalRequired(ER2265PosRePps090203.genConditionalField())));
		fs.add(sf(5, null, "Jangka Waktu",
				sv(C, 0, 255, all).confConditionalRequired(ER2265PosRePps090203.genConditionalField())));
		fs.add(sf(6, null, "Nama Tenaga Pendamping",
				sv(C, 0, 1000, all).confConditionalRequired(ER2265PosRePps090203.genConditionalField())));
		fs.add(sf(7, null, "Realisasi Program Alih Pengetahuan",
				sv(C, 0, 1000, all).confConditionalRequired(ER2265PosRePps090203.genConditionalField())));
		fs.add(sf(8, null, "Alasan",
				sv(C, 0, 4000, all).confConditionalRequired(ER2265PosRePps090203.genConditionalField())));
		return res;
	}
}
