...
import id.go.ojk.pnmk.client.builder.field.lb.reference.ER3014PosLbPnmk2800;
...
public class Lb2800 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 10, 10, ER3014PosLbPnmk2800.genFieldSave(), ER3014PosLbPnmk2800.genFieldSave());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLb.R3014_LB2800.getObject()).confRegex(patternNumeric)).confUnique(U));
		fs.add(sf(2, null, "Nominal", sv(M, 1, 16, numeric)));
		fs.add(sf(3, null, "CKPN", sv(M, 1, 16, numeric)));
		fs.add(sf(4, null, "Total", sv(M, 1, 16, numeric)).addFieldValidations(ER3014PosLbPnmk2800.genValidation4()));
		return res;
	}
}
