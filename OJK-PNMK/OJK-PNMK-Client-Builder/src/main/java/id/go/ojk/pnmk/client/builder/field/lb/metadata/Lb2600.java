...
import id.go.ojk.pnmk.client.builder.field.lb.reference.ER3012PosLbPnmk2600;
...
public class Lb2600 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		res.setSavePosForm(ER3012PosLbPnmk2600.genFieldSave());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLb.R3012_LB2600.getObject()).confRegex(patternNumeric)));
		...
		fs.add(sf(4, null, "Jangka Waktu, Tanggal Mulai", sv(M, 8, 8, date))
				.addFieldValidations(ER3012PosLbPnmk2600.genValidation4()));
		fs.add(sf(5, null, "Jangka Waktu, Tanggal Jatuh Tempo", sv(M, 8, 8, date))
				.addFieldValidations(ER3012PosLbPnmk2600.genValidation5()));
		fs.add(sf(6, null, "Suku Bunga/Diskonto", sv(M, 1, 6, numericDotNegatif)));
