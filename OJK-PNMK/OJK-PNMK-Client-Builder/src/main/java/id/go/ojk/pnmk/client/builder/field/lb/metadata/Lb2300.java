...
import id.go.ojk.pnmk.client.builder.field.lb.reference.ER3009PosLbPnmk2300;
...
public class Lb2300 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		res.setSavePosForm(ER3009PosLbPnmk2300.genFieldSave());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLb.R3009_LB2300.getObject()).confRegex(patternNumeric)));
		...
		fs.add(sf(7, null, "Jangka Waktu, Tanggal Mulai", sv(M, 8, 8, date))
				.addFieldValidations(ER3009PosLbPnmk2300.genValidation7()));
		fs.add(sf(8, null, "Jangka Waktu, Tanggal Jatuh Tempo", sv(M, 8, 8, date))
				.addFieldValidations(ER3009PosLbPnmk2300.genValidation8()));
		fs.add(sf(9, null, "Tingkat Suku Bunga (%)", sv(M, 1, 6, numericDotNegatif)));
