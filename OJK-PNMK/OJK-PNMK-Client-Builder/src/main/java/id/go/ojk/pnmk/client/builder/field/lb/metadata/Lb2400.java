...
import id.go.ojk.pnmk.client.builder.field.lb.reference.ER3010PosLbPnmk2400;
...
public class Lb2400 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLb.R3010_LB2400.getObject()).confRegex(patternNumeric)));
		...
		fs.add(sf(7, null, "Nama Debitur", sv(M, 1, 100, all)));
		...
		fs.add(sf(11, null, "Jangka Waktu, Tanggal Mulai", sv(M, 8, 8, date))
				.addFieldValidations(ER3010PosLbPnmk2400.genValidation11()));
		fs.add(sf(12, null, "Jangka Waktu, Tanggal Jatuh Tempo", sv(M, 8, 8, date))
				.addFieldValidations(ER3010PosLbPnmk2400.genValidation12()));
		fs.add(sf(13, null, "Plafon Pembiayaan/Pinjaman", sv(M, 1, 16, numeric)));
		fs.add(sf(14, null, "Suku Bunga/Imbal Hasil (%)", sv(M, 1, 6, numericDotNegatif)));
		fs.add(sf(15, null, "Outstanding Pembiayaan/Pinjaman", sv(M, 1, 16, numeric)));
		fs.add(sf(16, null, "Kualitas", sv(M, 1, 1, refTable).confReference(EHeaderMetadataShared.R006.getObject())
				.confRegex(patternNumeric.c())));
		fs.add(sf(17, null, "Dana Cadangan Nasabah", sv(M, 1, 16, numeric)));
		fs.add(sf(18, null, "Margin Ditangguhkan", sv(M, 1, 16, numeric)));
		return res;
	}
}
