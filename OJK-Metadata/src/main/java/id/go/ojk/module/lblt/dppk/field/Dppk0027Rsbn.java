...
import id.go.ojk.module.lblt.dppk.reference.ER7027PosLtlbDppkRsbn;
import id.go.ojk.module.lblt.dppk.validations.E7027RsbnValidationsConfig;
...
public enum Dppk0027Rsbn implements ILbltFieldMetadata {
    ...
    KODE_SURAT_BERHARGA(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(2, null, "Kode Surat Berharga",
                    sv(C, 1, 50, freeText)
                            .confConditionalRequired(E7027RsbnValidationsConfig.CR_EXISTS_POS_M))
    ),
    ...
    TANGGAL_JATUH_TEMPO(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(6, null, "Tanggal Jatuh Tempo",
                    sv(C, 8, 8, date)
                            .confConditionalRequired(E7027RsbnValidationsConfig.CR_EXISTS_POS_M))
                    .addFieldValidations(E7027RsbnValidationsConfig.FV_DATE_GREATER)
    ),
    ...
    SELISIH_PENILAIAN_INVESTASI_NILAI(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(9, null, "Selisih Penilaian Investasi Nilai",
                    sv(M, 1, 18, numeric))
                    .addFieldValidations(E7027RsbnValidationsConfig.FV_EQUAL_NILAI_INVESTASI)
    ),
    SELISIH_PENILAIAN_INVESTASI_PERSEN(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(10, null, "Selisih Penilaian Investasi %",
                    sv(C, 4, 6, numericDot)
                            .confConditionalRequired(E7027RsbnValidationsConfig.CR_EXISTS_POS_M))
                    .addFieldValidations(E7027RsbnValidationsConfig.FV_EQUAL_PERSENTASE_INVESTASI)
    ),
    ...
    PENGELOLAAN_NAMA_MANAJER_INVESTASI(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(14, null, "Pengelolaan Nama Manajer Investasi",
                    sv(C, 1, 250, freeText)
                            .confConditionalRequired(E7027RsbnValidationsConfig.CR_EXISTS_POS_M)
                            .confConditionalRequired(E7027RsbnValidationsConfig.CR_NAMA_PENGELOLA))
    ),
    KETERANGAN(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(15, null, "Keterangan",
                    sv(C, 1, 250, freeText)
                            .confConditionalRequired(E7027RsbnValidationsConfig.CR_EXISTS_POS_O))
    ),
    ;
    ...
    public static final LbltMetadataField<Dppk0027Rsbn> FIELD_KONVEN = new LbltMetadataField<>(Dppk0027Rsbn.class, Arrays.asList(KONVENSIONAL, SYARIAH), KODE_KOMPONEN_HEADERS);
    ...
    public static SubmissionFormat ppmpKonvensionalFormMetadata(ProgramType programType) {
        ...
        return new SubmissionConfig(programType.toString())
                .config()
                .setReferenceConfigs(ER7027PosLtlbDppkRsbn.Configs.REF_CONFIG_PPMP)
                .setSubmissionFormat(getPpmpSubmissionFormatConfig(KONVENSIONAL, programType.toString()))
                .setSubmissionField(FIELD_KONVEN.getFields())
                .setSegmentValidations(E7027RsbnValidationsConfig.VALIDATION_METADATA)
                .build()
                .get();
    }
