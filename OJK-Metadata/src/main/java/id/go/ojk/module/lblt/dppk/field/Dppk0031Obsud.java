...
import id.go.ojk.module.lblt.dppk.reference.ER7031PosLtlbDppkObsud;
import id.go.ojk.module.lblt.dppk.validations.E7031ObsudValidationsConfig;
...
public enum Dppk0031Obsud implements ILbltFieldMetadata {
    ...
    NAMA_PENERBIT(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(2, null, "Nama Penerbit",
                    sv(C, 1, 100, freeText)
                            .confConditionalRequired(E7031ObsudValidationsConfig.CR_EXISTS_POS_M))
    ),
    KODE_OBLIGASI_SUKUK_DAERAH(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(3, null, "Kode Obligasi/ Sukuk Daerah",
                    sv(C, 1, 30, alfaNumeric)
                            .confConditionalRequired(E7031ObsudValidationsConfig.CR_EXISTS_POS_M))
    ),
    NAMA_OBLIGASI_SUKUK_DAERAH(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
    ...
    TANGGAL_JATUH_TEMPO(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(8, null, "Tanggal Jatuh Tempo",
                    sv(C, 8, 8, date)
                            .confConditionalRequired(E7031ObsudValidationsConfig.CR_EXISTS_POS_M))
                    .addFieldValidations(E7031ObsudValidationsConfig.FV_DATE_GREATER)
    ),
    ...
    SELISIH_PENILAIAN_INVESTASI_NILAI(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(13, null, "Selisih Penilaian Investasi Nilai",
                    sv(M, 1, 18, numeric))
                    .addFieldValidations(E7031ObsudValidationsConfig.FV_EQUAL_NILAI_INVESTASI)
    ),
    SELISIH_PENILAIAN_INVESTASI_PERSEN(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(14, null, "Selisih Penilaian Investasi %",
                    sv(C, 4, 6, numericDot)
                            .confConditionalRequired(E7031ObsudValidationsConfig.CR_EXISTS_POS_M))
                    .addFieldValidations(E7031ObsudValidationsConfig.FV_EQUAL_PERSENTASE_INVESTASI)
    ),
    ...
    PENGELOLAAN_NAMA_MANAJER_INVESTASI(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(19, null, "Pengelolaan Nama Manajer Investasi",
                    sv(C, 1, 250, freeText)
                            .confConditionalRequired(E7031ObsudValidationsConfig.CR_EXISTS_POS_M)
                            .confConditionalRequired(E7031ObsudValidationsConfig.CR_NAMA_PENGELOLA))
    ),
    KETERANGAN(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(20, null, "Keterangan",
                    sv(C, 1, 250, freeText)
                            .confConditionalRequired(E7031ObsudValidationsConfig.CR_EXISTS_POS_O))
    ),
    ;
    ...
    public static final LbltMetadataField<Dppk0031Obsud> FIELD_KONVEN = new LbltMetadataField<>(Dppk0031Obsud.class, Arrays.asList(KONVENSIONAL, SYARIAH), KODE_KOMPONEN_HEADERS);
    ...
    public static SubmissionFormat ppmpKonvensionalFormMetadata(ProgramType programType) {
        ...
        return new SubmissionConfig(programType.toString())
                .config()
                .setReferenceConfigs(ER7031PosLtlbDppkObsud.Configs.REF_CONFIG_PPMP)
                .setSubmissionFormat(getPpmpSubmissionFormatConfig(KONVENSIONAL, programType.toString()))
                .setSubmissionField(FIELD_KONVEN.getFields())
                .setSegmentValidations(E7031ObsudValidationsConfig.VALIDATION_METADATA)
                .build()
                .get();
    }
