...
import id.go.ojk.module.lblt.dppk.reference.ER7040PosLtlbDppkProp;
import id.go.ojk.module.lblt.dppk.validations.E7040PropValidationsConfig;
...
public enum Dppk0040Prop implements ILbltFieldMetadata {
    ...
    JENIS_OBJEK_TANAH_BANGUNAN_TANAH_BANGUNAN(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
    ...
    ALAMAT_LENGKAP(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(3, null, "Alamat Lengkap",
                    sv(C, 1, 250, freeText)
                            .confConditionalRequired(E7040PropValidationsConfig.CR_EXISTS_POS_M))),

    LUAS_M2(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(4, null, "Luas(m2)",
                    sv(C, 1, 10, numeric)
                            .confConditionalRequired(E7040PropValidationsConfig.CR_EXISTS_POS_M))),

    JENIS_BUKTI_KEPEMILIKAN(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
    ...
    NOMOR_SURAT_KEPEMILIKAN(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
    ...
    NILAI_APPRAISAL_NILAI_WAJAR(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(11, null, "Nilai Appraisal/ Nilai Wajar",
                    sv(M, 1, 18, numeric))),

    TANGGAL_APPRAISAL(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(12, null, "Tanggal Appraisal",
                    sv(C, 8, 8, date)
                            .confConditionalRequired(E7040PropValidationsConfig.CR_EXISTS_POS_M))),

    NAMA_PENILAI_PUBLIK_INDEPENDEN(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
            sf(13, null, "Nama Penilai Publik/Independen",
                    sv(C, 1, 100, freeText)
                            .confConditionalRequired(E7040PropValidationsConfig.CR_EXISTS_POS_M))),

    NAMA_KJPP(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
    ...
    PENDAPATAN_SEWA(sectors(KONVENSIONAL, SYARIAH), programs(PPMPK, PPMPM),
    ...
    public static final LbltMetadataField<Dppk0040Prop> FIELD_KONVEN = new LbltMetadataField<>(Dppk0040Prop.class, Arrays.asList(KONVENSIONAL, SYARIAH), KODE_KOMPONEN_HEADERS);
    ...
    public static SubmissionFormat ppmpKonvensionalFormMetadata(ProgramType programType) {
        ...
        return new SubmissionConfig(programType.toString())
                .config()
                .setReferenceConfigs(ER7040PosLtlbDppkProp.Configs.REF_CONFIG_PPMP)
                .setSubmissionFormat(getPpmpSubmissionFormatConfig(KONVENSIONAL, programType.toString()))
                .setSubmissionField(FIELD_KONVEN.getFields())
                .setSegmentValidations(E7040PropValidationsConfig.VALIDATION_METADATA)
                .build()
                .get();
    }
