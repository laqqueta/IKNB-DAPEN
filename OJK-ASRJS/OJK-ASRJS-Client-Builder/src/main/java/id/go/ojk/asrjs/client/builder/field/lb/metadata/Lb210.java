...
import id.go.ojk.asrjs.client.builder.field.lb.reference.ER3018PosLbAsrjs210;
...
public class Lb210 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode,
				new ArrayList<>(), extension, 3, 3, ER3018PosLbAsrjs210.genFieldSave(), null,
				ER3018PosLbAsrjs210.getRequiredPos());
		res.addSegmentValidations(ER3018PosLbAsrjs210.genValidationTotalRisikoKredit());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLb.R3018_LB210.getObject())).confUnique(U));
		fs.add(sf(2, null, "Dana Tabarru dan Dana Tanahud", sv(M, 1, 16, numericNegatif)));
		fs.add(sf(3, null, "Dana Perusahaan",
				sv(C, 1, 16, numericNegatif).confConditionalRequired(ER3018PosLbAsrjs210.genConditional3())));
