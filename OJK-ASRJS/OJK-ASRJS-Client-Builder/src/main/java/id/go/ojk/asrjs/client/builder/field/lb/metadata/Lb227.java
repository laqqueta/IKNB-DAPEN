...
import id.go.ojk.asrjs.client.builder.field.lb.reference.ER3035PosLbAsrjs227;
...
public class Lb227 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		fs.add(sf(3, null, "Jenis Utang", sv(C, 1, 4, refTable).confReference(EHeaderMetadataShared.R022.getObject())
				.confConditionalRequired(ER3035PosLbAsrjs227.genConditional())));
		...
		fs.add(sf(5, null, "Periode Mulai",
				sv(C, 1, 8, date).confConditionalRequired(ER3035PosLbAsrjs227.genConditional())));
		fs.add(sf(6, null, "Mata Uang", sv(C, 1, 4, refTable).confReference(EHeaderMetadataShared.R017.getObject())
				.confConditionalRequired(ER3035PosLbAsrjs227.genConditional())));
		fs.add(sf(7, null, "Saldo Buku",
				sv(C, 1, 10, numericNegatif).confConditionalRequired(ER3035PosLbAsrjs227.genConditional())));
		fs.add(sf(8, null, "Saldo Penilaian SAP",
				sv(C, 1, 10, numericNegatif).confConditionalRequired(ER3035PosLbAsrjs227.genConditional())));
		return res;
	}
}
