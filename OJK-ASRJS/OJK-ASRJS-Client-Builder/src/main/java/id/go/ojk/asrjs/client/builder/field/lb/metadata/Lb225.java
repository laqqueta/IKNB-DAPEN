...
import id.go.ojk.asrjs.client.builder.field.lb.reference.ER3033PosLbAsrjs225;
...
public class Lb225 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		fs.add(sf(2, null, "Nama Debitur", sv(O, 1, 100, alfaSpace)));
		fs.add(sf(3, null, "Kategori Usaha",
				sv(C, 1, 4, refTable).confReference(EHeaderMetadataShared.R011.getObject())
						.confConditionalRequired(ER3033PosLbAsrjs225.genConditional())));
		...
		fs.add(sf(6, null, "Periode Mulai",
				sv(C, 1, 8, date).confConditionalRequired(ER3033PosLbAsrjs225.genConditional())));
		...
		fs.add(sf(9, null, "Saldo Buku",
				sv(C, 1, 16, numericNegatif).confConditionalRequired(ER3033PosLbAsrjs225.genConditional())));
