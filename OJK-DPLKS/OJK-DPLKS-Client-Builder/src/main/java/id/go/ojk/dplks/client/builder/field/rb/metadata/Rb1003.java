...
import id.go.ojk.dplks.client.builder.field.rb.reference.ER2014PosRbDplks1003;
...
public class Rb1003 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataRb.R2014_RB1003.getObject())));
		fs.add(sf(2, null, "Materi/Topik", sv(O, 0, 200, all)));
		fs.add(sf(3, null, "Nama Peserta",
				sv(C, 1, 150, all).confConditionalRequired(ER2014PosRbDplks1003.genConditionalAllExist())));
		fs.add(sf(4, null, "Jabatan",
				sv(C, 1, 150, all).confConditionalRequired(ER2014PosRbDplks1003.genConditionalAllExist())));
		fs.add(sf(5, null, "Sertifikasi",
				sv(C, 1, 100, all).confConditionalRequired(ER2014PosRbDplks1003.genConditionalAllExist())));
		fs.add(sf(6, null, "Waktu Pelatihan",
				sv(C, 1, 50, all).confConditionalRequired(ER2014PosRbDplks1003.genConditionalAllExist())));
		return res;
	}
}
