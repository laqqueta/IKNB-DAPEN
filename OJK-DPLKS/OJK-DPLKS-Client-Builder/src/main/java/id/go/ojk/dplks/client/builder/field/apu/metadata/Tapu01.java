...
import id.go.ojk.dplks.client.builder.field.EFormApuPenilaian;
import id.go.ojk.dplks.client.builder.field.apu.reference.EHeaderMetadataPenilaian;
import id.go.ojk.dplks.client.builder.field.apu.reference.ER5000PosApuDplks01;
...
public class Tapu01 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode,
				new ArrayList<>(), extension, 43, 43, null, null, ER5000PosApuDplks01.getRequiredPos());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 16, 16, refTable).confReference(EHeaderMetadataPenilaian.R000.getObject())).confUnique(U));
		fs.add(sf(2, null, "Jumlah Peserta", sv(M, 1, 16, numeric)));
		fs.add(sf(3, null, "Jumlah Nominal Dana/Aset Investasi", sv(M, 1, 16, numeric)));
		return res;
	}
}
