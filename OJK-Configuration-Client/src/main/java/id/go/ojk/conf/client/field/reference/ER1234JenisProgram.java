package id.go.ojk.conf.client.field.reference;

import java.util.ArrayList;
import java.util.List;

import id.go.ojk.lib.client.IObject;
import id.go.ojk.lib.client.model.KeyValueString;
import lombok.AccessLevel;
import lombok.AllArgsConstructor;

@AllArgsConstructor(access = AccessLevel.PRIVATE)
public enum ER1234JenisProgram implements IObject<KeyValueString> {
  R1 ("BDN", "Badan"),
  R2 ("JKK", "Jaminan Kecelakaan Kerja"),
  R3 ("JK", "Jaminan Kematian"),
  R4 ("JHT", "Jaminan Hari Tua"),
  R5 ("JP", "Jaminan Pensiun"),
  R6 ("JKP", "Jaminan Kehilangan Pekerjaan"),
  ;

  private String key;
  private String value;

  public KeyValueString getObject() {
    return new KeyValueString(key, value, new String[] {});
  }

  public static List<KeyValueString> getObjects() {
    List<KeyValueString> res = new ArrayList<>();
    for (ER1234JenisProgram eEnum : ER1234JenisProgram.values()) {
      res.add(eEnum.getObject());
    }
    return res;
  }

  public static String getName() {
    return ER1234JenisProgram.class.getSimpleName().substring(6);
  }

  public static int getRefNumber() {
    return Integer.parseInt(ER1234JenisProgram.class.getSimpleName().substring(2, 6));
  }
}
