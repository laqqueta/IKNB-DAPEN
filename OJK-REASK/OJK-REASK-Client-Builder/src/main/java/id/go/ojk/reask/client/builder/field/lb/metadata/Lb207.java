...
import id.go.ojk.reask.client.builder.field.lb.reference.ER3038PosLbReask207;
...
public class Lb207 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 1, null, ER3038PosLbReask207.genFieldSave(), null, ER3038PosLbReask207.getRequiredPos());
		res.addSegmentValidations(ER3038PosLbReask207.genValidationTotal());
		...
		fs.add(sf(2, null, "Uraian",
				sv(C, 1, 50, all).confConditionalRequired(ER3038PosLbReask207.genConditionalField2())));
		fs.add(sf(3, null, "Hasil Lain - Kegiatan Usaha Berbasis Imbalan Jasa (Fee Based) - Jasa Administrasi (ASO)",
				sv(C, 1, 16, numericNegatif).confConditionalRequired(ER3038PosLbReask207.genConditionalField())));
		...
		fs.add(sf(7, null, "Total Hasil Lain",
				sv(C, 1, 16, numericNegatif).confConditionalRequired(ER3038PosLbReask207.genConditionalField()))
				.addFieldValidations(ER3038PosLbReask207.genFieldValidation7()));
		fs.add(sf(8, null, "Beban Lain",
				sv(C, 1, 16, numericNegatif).confConditionalRequired(ER3038PosLbReask207.genConditionalField())));
		fs.add(sf(9, null, "Jumlah",
				sv(C, 1, 16, numericNegatif).confConditionalRequired(ER3038PosLbReask207.genConditionalField()))
				.addFieldValidations(ER3038PosLbReask207.genFieldValidation9()));
		return res;
	}
}
