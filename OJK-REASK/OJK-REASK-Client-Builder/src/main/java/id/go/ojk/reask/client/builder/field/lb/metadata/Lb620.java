...
import id.go.ojk.reask.client.builder.field.lb.reference.ER3059PosLbReask620;
...
public class Lb620 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLb.R3059_LB620.getObject())));
		fs.add(sf(2, null, "Nama Subdana", sv(O, 1, 50, all)));
		fs.add(sf(3, null, "Mata Uang", sv(C, 1, 6, refTable).confReference(EHeaderMetadataShared.R016.getObject())
				.confConditionalRequired(ER3059PosLbReask620.genConditionField())));
		fs.add(sf(4, null, "Kurs",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3059PosLbReask620.genConditionField())));
		fs.add(sf(5, null, "NAB",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3059PosLbReask620.genConditionField())));
		fs.add(sf(6, null, "NAB/Unit",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3059PosLbReask620.genConditionField())));
		fs.add(sf(7, null, "Hasil Investasi Tahunan",
				sv(C, 1, 16, numericDot).confConditionalRequired(ER3059PosLbReask620.genConditionField())));
