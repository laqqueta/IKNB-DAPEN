...
import id.go.ojk.reask.client.builder.field.rb.reference.ER2018PosRbReask1313;
...
public class Rb1313 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 7, null, null, null, ER2018PosRbReask1313.getRequiredPos());
		res.addSegmentValidations(ER2018PosRbReask1313.genValidationMaxLengthNumeric());
		res.addSegmentValidations(ER2018PosRbReask1313.genValidationNumeric());
		res.addSegmentValidations(ER2018PosRbReask1313.genValidationNumericDot());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataRb.R2018_RB1313.getObject())));
		fs.add(sf(2, null, "Asumsi",
				sv(O, 1, 1000, all).confConditionalRequired(ER2018PosRbReask1313.genConditionalField())));
