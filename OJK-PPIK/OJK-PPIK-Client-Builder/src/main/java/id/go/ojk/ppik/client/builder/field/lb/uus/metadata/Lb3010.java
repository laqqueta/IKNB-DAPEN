...
import id.go.ojk.ppik.client.builder.field.lb.uus.reference.ER3122PosLbPpiu3010;
...
public class Lb3010 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulananUus eEnum = EFormLaporanBulananUus.LB_3010;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		res.setSavePosForm(ER3122PosLbPpiu3010.genFieldSave());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLbUus.R3123_LB3010.getObject())
		...
		fs.add(sf(3, null, "Nominal Underlying Transaksi Pinjaman/ Pendanaan",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3122PosLbPpiu3010.genFieldConditionA())));
		fs.add(sf(4, null, "Nomor Kontrak Instrumen Derivatif",
				sv(C, 1, 35, all).confConditionalRequired(ER3122PosLbPpiu3010.genFieldConditionA())));
		fs.add(sf(5, null, "Jenis Instrumen Derivatif",
				sv(C, 1, 10, refTable).confConditionalRequired(ER3122PosLbPpiu3010.genFieldConditionA())
		...
		fs.add(sf(7, null, "Tanggal Mulai",
				sv(C, 8, 8, date).confConditionalRequired(ER3122PosLbPpiu3010.genFieldConditionA()))
						.addFieldValidations(ER3122PosLbPpiu3010.genFieldValidation7()));
		...
		fs.add(sf(9, null, "Nominal Instrumen Derivatif untuk Lindung Nilai (Dalam Mata Uang Asal)",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3122PosLbPpiu3010.genFieldConditionB())));
		...
		fs.add(sf(11, null, "Nama Counterparty", sv(C, 1, 100, all)
				.confConditionalRequired(ER3122PosLbPpiu3010.genFieldConditionA())));
