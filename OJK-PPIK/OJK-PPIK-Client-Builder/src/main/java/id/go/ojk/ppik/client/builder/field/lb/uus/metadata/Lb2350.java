...
import id.go.ojk.ppik.client.builder.field.lb.uus.reference.ER3116PosLbPpiu2350;
...
public class Lb2350 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulananUus eEnum = EFormLaporanBulananUus.LB_2350;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		res.setSavePosForm(ER3116PosLbPpiu2350.genFieldSave());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLbUus.R3117_LB2350.getObject())
						.confRegex(patternNumeric)));
		fs.add(sf(2, null, "Nama Penerima", sv(M, 1, 100, all)));
		fs.add(sf(3, null, "Status Usaha",
				sv(C, 1, 10, refTable).confConditionalRequired(ER3116PosLbPpiu2350.genFieldCondition())
		...
		fs.add(sf(7, null, "Nilai Proyek Awal, Dalam Nilai Mata Uang Asal",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3116PosLbPpiu2350.genFieldCondition())));
		...
		fs.add(sf(11, null, "Jangka Waktu, Tanggal Mulai",
				sv(C, 8, 8, date).confConditionalRequired(ER3116PosLbPpiu2350.genFieldCondition()))
						.addFieldValidations(ER3116PosLbPpiu2350.genFieldValidation11()));
