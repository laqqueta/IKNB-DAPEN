...
import id.go.ojk.ppik.client.builder.field.lb.uus.reference.ER3107PosLbPpiu0046;
...
public class Lb0046 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulananUus eEnum = EFormLaporanBulananUus.LB_0046;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLbUus.R3108_LB0046.getObject())
						.confRegex(patternNumeric)));
		fs.add(sf(2, null, "Nama", sv(C, 1, 100, all).confConditionalRequired(ER3107PosLbPpiu0046.genFieldCondition())
				.confRegex(PATTERN_NAME.c())));
		fs.add(sf(3, null, "Nomor Identitas",
				sv(C, 1, 20, alfaNumeric).confConditionalRequired(ER3107PosLbPpiu0046.genFieldCondition())));
		fs.add(sf(4, null, "Kewarganegaraan",
				sv(O, 1, 10, refTable).confReference(EHeaderMetadataShared.R024.getObject())
						.confRegex(PATTERN_REFERENCE_1.c()))
								.addFieldValidations(ER3107PosLbPpiu0046.genFieldValidation4()));
		fs.add(sf(5, null, "Jabatan",
				sv(C, 1, 10, refTable).confConditionalRequired(ER3107PosLbPpiu0046.genFieldCondition())
		...
		fs.add(sf(8, null, "Nomor Izin Kerja",
				sv(C, 1, 25, all).confConditionalRequired(ER3107PosLbPpiu0046.genFieldCondition())));
		fs.add(sf(9, null, "Tanggal Izin Kerja",
				sv(C, 8, 8, date).confConditionalRequired(ER3107PosLbPpiu0046.genFieldCondition())));
		fs.add(sf(10, null, "Awal Masa Laku Izin Kerja",
				sv(C, 8, 8, date).confConditionalRequired(ER3107PosLbPpiu0046.genFieldCondition()))
						.addFieldValidations(ER3107PosLbPpiu0046.genFieldValidation10()));
