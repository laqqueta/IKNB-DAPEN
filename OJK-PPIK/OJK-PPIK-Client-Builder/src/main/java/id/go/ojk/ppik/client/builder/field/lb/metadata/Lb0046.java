...
import id.go.ojk.ppik.client.builder.field.lb.reference.ER3008PosLbPpik0046;
...
public class Lb0046 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLb.R3008_LB0046.getObject()).confRegex(patternNumeric)));
		fs.add(sf(2, null, "Nama", sv(C, 1, 100, all).confConditionalRequired(ER3008PosLbPpik0046.genFieldCondition())
				.confRegex(PATTERN_NAME.c())));
		fs.add(sf(3, null, "Nomor Identitas",
				sv(C, 1, 20, alfaNumeric).confConditionalRequired(ER3008PosLbPpik0046.genFieldCondition())));
		fs.add(sf(4, null, "Kewarganegaraan",
				sv(O, 1, 10, refTable).confReference(EHeaderMetadataShared.R024.getObject())
						.confRegex(PATTERN_REFERENCE_1.c()))
								.addFieldValidations(ER3008PosLbPpik0046.genFieldValidation4()));
		fs.add(sf(5, null, "Jabatan",
				sv(C, 1, 10, refTable).confConditionalRequired(ER3008PosLbPpik0046.genFieldCondition())
		...
		fs.add(sf(8, null, "Nomor Izin Kerja",
				sv(C, 1, 25, all).confConditionalRequired(ER3008PosLbPpik0046.genFieldCondition())));
		fs.add(sf(9, null, "Tanggal Izin Kerja",
				sv(C, 8, 8, date).confConditionalRequired(ER3008PosLbPpik0046.genFieldCondition())));
		fs.add(sf(10, null, "Awal Masa Laku Izin Kerja",
				sv(C, 8, 8, date).confConditionalRequired(ER3008PosLbPpik0046.genFieldCondition()))
						.addFieldValidations(ER3008PosLbPpik0046.genFieldValidation10()));
		fs.add(sf(11, null, "Akhir Masa Laku Izin Kerja",
				sv(C, 8, 8, date).confConditionalRequired(ER3008PosLbPpik0046.genFieldCondition()))
						.addFieldValidations(ER3008PosLbPpik0046.genFieldValidation11()));
		return res;
	}
}
