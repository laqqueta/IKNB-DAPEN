...
import id.go.ojk.ppik.client.builder.field.lb.uus.reference.ER3115PosLbPpiu2300;
...
public class Lb2300 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		res.setSavePosForm(ER3115PosLbPpiu2300.genFieldSave());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLbUus.R3116_LB2300.getObject()).confRegex(patternNumeric)));
		...
		fs.add(sf(7, null, "Persentase Bagian Penyertaan", sv(M, 1, 6, numericDot))
				.addFieldValidations(ER3115PosLbPpiu2300.genFieldValidation7()));
		...
		fs.add(sf(9, null, "Nilai Penyertaan Awal, Dalam Nilai Mata Uang Asal",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3115PosLbPpiu2300.genFieldCondition())));
