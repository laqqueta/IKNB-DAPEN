...
import id.go.ojk.ppik.client.builder.field.lb.uus.reference.ER3120PosLbPpiu2600;
...
public class Lb2600 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		res.setSavePosForm(ER3120PosLbPpiu2600.genFieldSave());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris",
				sv(M, 10, 10, refTable).confReference(EHeaderMetadataLbUus.R3121_LB2600.getObject())
		...
		fs.add(sf(6, null, "Jangka Waktu, Tanggal Mulai", sv(M, 8, 8, date))
				.addFieldValidations(ER3120PosLbPpiu2600.genFieldValidation6()));
		fs.add(sf(7, null, "Jangka Waktu, Tanggal Jatuh Tempo",
				sv(M, 8, 8, date).confConditionalRequired(ER3120PosLbPpiu2600.genFieldConditionA())));
		...
		fs.add(sf(11, null, "Nilai Nominal Surat Berharga, Dalam Mata Uang Asal",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3120PosLbPpiu2600.genFieldConditionB())));
		...
		fs.add(sf(15, null, "Nama Investor", sv(M, 1, 100, all)));
		...
		fs.add(sf(17, null, "Golongan Kreditur", sv(M, 1, 10, refTable)
		...
		fs.add(sf(21, null, "Nama Wali Amanat", sv(M, 1, 30, all)));
		fs.add(sf(22, null, "Lembaga Pemeringkat",
				sv(C, 1, 10, refTable).confConditionalRequired(ER3120PosLbPpiu2600.genFieldConditionC())
		...
		fs.add(sf(24, null, "Tanggal Pemeringkatan",
				sv(C, 8, 8, date).confConditionalRequired(ER3120PosLbPpiu2600.genFieldConditionC())));
		return res;
	}
}
