...
import id.go.ojk.ppik.client.builder.field.lb.uus.reference.ER3102PosLbPpiu0020;
...
public class Lb0020 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		res.setSavePosForm(ER3102PosLbPpiu0020.genFieldSave());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLbUus.R3102_LB0020.getObject()).confRegex(patternNumeric)));
		...
		fs.add(sf(3, null, "Tanggal Pencatatan Pelaporan Kantor Cabang",
				sv(C, 8, 8, date).confConditionalRequired(ER3102PosLbPpiu0020.genFieldCondition())));
		fs.add(sf(4, null, "Nama Kantor Cabang", sv(C, 1, 100, all)
				.confConditionalRequired(ER3102PosLbPpiu0020.genFieldCondition())));
		fs.add(sf(5, null, "Alamat",
				sv(C, 1, 200, all).confConditionalRequired(ER3102PosLbPpiu0020.genFieldCondition())));
		fs.add(sf(6, null, "Kecamatan", sv(C, 1, 100, all)
				.confConditionalRequired(ER3102PosLbPpiu0020.genFieldCondition()).confRegex(patternAlfaNumericSpace)));
		...
		fs.add(sf(8, null, "Kode Pos",
				sv(C, 1, 5, numeric).confConditionalRequired(ER3102PosLbPpiu0020.genFieldCondition())));
		fs.add(sf(9, null, "Nomor Telepon",
				sv(C, 1, 20, numeric).confConditionalRequired(ER3102PosLbPpiu0020.genFieldCondition())));
		fs.add(sf(10, null, "Jumlah Tenaga Kerja Kantor Cabang",
				sv(C, 1, 5, numeric).confConditionalRequired(ER3102PosLbPpiu0020.genFieldCondition())));
		fs.add(sf(11, null, "Nama Kepala Cabang", sv(C, 1, 100, all)
				.confConditionalRequired(ER3102PosLbPpiu0020.genFieldCondition()).confRegex(PATTERN_NAME)));
		return res;
	}
}
