...
import id.go.ojk.ppik.client.builder.field.lb.reference.ER3015PosLbPpik2200;
...
public class Lb2200 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		res.setSavePosForm(ER3015PosLbPpik2200.genFieldSave());
		...
		fs.add(sf(4, null, "Informasi Surat Berharga yang Dimiliki, Jangka Waktu, Tanggal Mulai", sv(M, 8, 8, date))
				.addFieldValidations(ER3015PosLbPpik2200.genFieldValidation4()));
		fs.add(sf(5, null, "Informasi Surat Berharga yang Dimiliki, Jangka Waktu, Tanggal Jatuh Tempo",
				sv(C, 8, 8, date).confConditionalRequired(ER3015PosLbPpik2200.genFieldConditionA())));
		fs.add(sf(6, null, "Informasi Surat Berharga yang Dimiliki, Jenis Valuta", sv(M, 1, 10, refTable)
				.confReference(EHeaderMetadataShared.R020.getObject()).confRegex(PATTERN_REFERENCE_1.c())));
		fs.add(sf(7, null, "Informasi Surat Berharga yang Dimiliki, Tujuan Kepemilikan", sv(M, 1, 10, refTable)
				.confReference(EHeaderMetadataShared.R023.getObject()).confRegex(PATTERN_REFERENCE_1.c())));
		fs.add(sf(8, null, "Informasi Surat Berharga yang Dimiliki, Jenis Suku Bunga/ Imbal Hasil",
				sv(M, 1, 10, refTable).confReference(EHeaderMetadataShared.R015.getObject())
						.confRegex(PATTERN_REFERENCE_1.c())));
		fs.add(sf(9, null, "Informasi Surat Berharga yang Dimiliki, Nilai Suku Bunga/ Imbal Hasil",
				sv(M, 1, 16, numeric)));
		fs.add(sf(10, null, "Informasi Surat Berharga yang Dimiliki, Tingkat Suku Bunga/Imbal Hasil",
				sv(M, 1, 6, numericDot)));
		fs.add(sf(11, null, "Saldo Akhir, Dalam Mata Uang Asal", sv(M, 1, 16, numeric)));
		fs.add(sf(12, null, "Saldo Akhir, Dalam Ekuivalen Rupiah", sv(M, 1, 16, numeric)));
		fs.add(sf(13, null, "Informasi Perusahaan Penerbit, Nama Penerbit",
				sv(M, 1, 100, all).confRegex(PATTERN_NAME.c())));
		fs.add(sf(14, null, "Informasi Perusahaan Penerbit, Lokasi Negara Penerbit", sv(M, 1, 10, refTable)
				.confReference(EHeaderMetadataShared.R024.getObject()).confRegex(PATTERN_REFERENCE_1.c())));
		fs.add(sf(15, null, "Informasi Perusahaan Penerbit, Golongan Penerbit", sv(M, 1, 10, refTable)
				.confReference(EHeaderMetadataShared.R031.getObject()).confRegex(PATTERN_REFERENCE_1.c())));
		fs.add(sf(16, null, "Informasi Perusahaan Penerbit, Status Keterkaitan", sv(M, 1, 10, refTable)
				.confReference(EHeaderMetadataShared.R014.getObject()).confRegex(PATTERN_REFERENCE_1.c())));
		fs.add(sf(17, null, "Lembaga Pemeringkat",
				sv(C, 1, 10, refTable).confConditionalRequired(ER3015PosLbPpik2200.genFieldConditionB())
						.confReference(EHeaderMetadataShared.R025.getObject()).confRegex(PATTERN_REFERENCE_1.c())));
		fs.add(sf(18, null, "Peringkat Surat Berharga",
				sv(C, 1, 10, refTable).confConditionalRequired(ER3015PosLbPpik2200.genFieldConditionB())
						.confReference(EHeaderMetadataShared.R026.getObject()).confRegex(PATTERN_REFERENCE_1.c())));
		fs.add(sf(19, null, "Tanggal Pemeringkatan",
				sv(C, 8, 8, date).confConditionalRequired(ER3015PosLbPpik2200.genFieldConditionB())));
		return res;
	}
}
