...
import id.go.ojk.ppik.client.builder.field.lb.reference.ER3003PosLbPpik0030;
...
public class Lb0030 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 1, null, ER3003PosLbPpik0030.genFieldSave(), ER3003PosLbPpik0030.genFieldSave());
		res.addSegmentValidations(ER3003PosLbPpik0030.genRowValidation10());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLb.R3003_LB0030.getObject()).confRegex(patternNumeric)));
		...
		fs.add(sf(7, null, "Ekuitas Pemegang Saham (dalam Rp)",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3003PosLbPpik0030.genFieldConditionB())));
		fs.add(sf(8, null, "Persentase Kepemilikan Asing secara Langsung/Tidak Langsung", sv(M, 1, 6, numericDot))
				.addFieldValidations(ER3003PosLbPpik0030.genFieldValidation08()));
		...
		fs.add(sf(11, null, "Nama Pengurus Pemegang Saham", sv(C, 1, 100, all)
				.confConditionalRequired(ER3003PosLbPpik0030.genFieldConditionA()).confRegex(PATTERN_NAME.c())));
		fs.add(sf(12, null, "Jabatan Pengurus Pemegang Saham",
				sv(C, 1, 10, refTable).confConditionalRequired(ER3003PosLbPpik0030.genFieldConditionA())
		...
		fs.add(sf(17, null, "Nilai Kepemilikan Saham Derajat Kedua",
				sv(C, 1, 16, numeric).confConditionalRequired(ER3003PosLbPpik0030.genFieldConditionA())));
		return res;
	}
}
