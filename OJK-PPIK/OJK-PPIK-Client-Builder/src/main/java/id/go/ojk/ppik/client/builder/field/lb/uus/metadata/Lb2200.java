...
import id.go.ojk.ppik.client.builder.field.lb.uus.reference.ER3114PosLbPpiu2200;
...
public class Lb2200 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		...
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 0, null);
		res.setSavePosForm(ER3114PosLbPpiu2200.genFieldSave());
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLbUus.R3115_LB2200.getObject()).confRegex(patternNumeric)));
		...
		fs.add(sf(4, null, "Informasi Surat Berharga yang Dimiliki, Jangka Waktu, Tanggal Mulai", sv(M, 8, 8, date))
				.addFieldValidations(ER3114PosLbPpiu2200.genFieldValidation04()));
		fs.add(sf(5, null, "Informasi Surat Berharga yang Dimiliki, Jangka Waktu, Tanggal Jatuh Tempo",
				sv(C, 8, 8, date).confConditionalRequired(ER3114PosLbPpiu2200.genFieldConditionA())));
		...
		fs.add(sf(10, null, "Informasi Surat Berharga yang Dimiliki, Nilai Imbal Hasil", sv(M, 1, 16, numeric)));
		...
		fs.add(sf(18, null, "Lembaga Pemeringkat",
				sv(C, 1, 10, refTable).confConditionalRequired(ER3114PosLbPpiu2200.genFieldConditionB())
		...
		fs.add(sf(20, null, "Tanggal Pemeringkatan",
				sv(C, 8, 8, date).confConditionalRequired(ER3114PosLbPpiu2200.genFieldConditionB())));
		return res;
	}
}
