...
import id.go.ojk.ppik.client.builder.field.lb.uus.reference.ER3103PosLbPpiu0035;
...
public class Lb0035 extends BaseMetadata {
	...
	public SubmissionFormat get() {
		EFormLaporanBulananUus eEnum = EFormLaporanBulananUus.LB_0035;
		SubmissionFormat res = new SubmissionFormat(eEnum.getCode(), eEnum.getName(), reportCode, new ArrayList<>(),
				extension, 1, null);
		List<SubmissionField> fs = res.getFields();
		fs.add(sf(0, null, "Flag Detail", sv(M, 3, 3, alfaNumeric).confConstant("D01")));
		fs.add(sf(1, null, "Kode Komponen / Baris", sv(M, 10, 10, refTable)
				.confReference(EHeaderMetadataLbUus.R3104_LB0035.getObject()).confRegex(patternNumeric)));
		...
		fs.add(sf(4, null, "Kewarganegaraan",
				sv(M, 1, 10, refTable).confReference(EHeaderMetadataShared.R024.getObject())
						.confRegex(PATTERN_REFERENCE_1.c()))
								.addFieldValidations(ER3103PosLbPpiu0035.genFieldValidation4()));
		...
		fs.add(sf(6, null, "Domisili", sv(C, 1, 10, refTable).confReference(EHeaderMetadataShared.R033.getObject())
				.confRegex(PATTERN_REFERENCE_1.c()).confConditionalRequired(ER3103PosLbPpiu0035.genFieldCondition())));
		...
		fs.add(sf(9, null, "Tanggal Mulai Menjabat", sv(M, 8, 8, date))
				.addFieldValidations(ER3103PosLbPpiu0035.genFieldValidation9()));
